...
class CopyTaskGenerator:
    ...
    def generate(
        ...
        # Input structure: [start_marker, sequence, delimiter, delay, output]
        # Sequence phases:
        # 1. Start marker (1 step)
        # 2. Input sequence (seq_len steps)
        # 3. Delimiter (1 step)
        # 4. Delay (delay steps)
        # 5. Output sequence (seq_len steps)
        # Total length = 1 + seq_len + 1 + delay + seq_len
        total_features = vec_size + 2
        total_seq_len = seq_len * 2 + delay + 2

        inputs = np.zeros((num_samples, total_seq_len, total_features), dtype=np.float32)
        targets = np.zeros((num_samples, total_seq_len, vec_size), dtype=np.float32)
        masks = np.zeros((num_samples, total_seq_len), dtype=np.float32)
        ...
        for i in range(num_samples):
            ...
            # Index logic:
            # 0: Start
            # 1 to seq_len: Sequence
            # seq_len + 1: Delimiter
            # seq_len + 2 to seq_len + 1 + delay: Delay
            # seq_len + delay + 2: Start of output

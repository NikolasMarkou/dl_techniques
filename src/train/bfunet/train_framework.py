"""Training script for BFU-Net with deep supervision and synthesis monitoring."""
...
class BFUNetConfig(TrainingConfig):
    """Configuration for BFU-Net with deep supervision."""
...
class BFUNetDatasetBuilder(DatasetBuilder):
    ...
    def _load_and_preprocess_image(self, image_path: tf.Tensor) -> tf.Tensor:
        try:
            ...
            image = tf.image.decode_image(
                image_string, channels=self.config.channels, expand_animations=False
            )
    ...
    def _create_multiscale_labels(
            self, noisy_patch: tf.Tensor, clean_patch: tf.Tensor
...
def unconditional_sampling(
    ...
    """Generate images using implicit prior sampling (Kadkhodaie & Simoncelli)."""
    if seed is not None:
...
class SynthesisMonitorCallback(keras.callbacks.Callback):
    ...
    def _visualize_synthesis(self, epoch: int, final: tf.Tensor, intermediates: List[tf.Tensor]):
        try:
...
class BFUNetTrainingPipeline(TrainingPipeline):
    """Training pipeline for BFU-Net with deep supervision."""
    ...
    def _create_callbacks(
        ...
        callbacks = super()._create_callbacks(lr_schedule, custom_callbacks, train_ds)
...
def main():
    ...
    logger.info(f"BFU-Net: variant={config.model_args['variant']}, "
                f"DS={config.enable_deep_supervision}, synthesis={config.enable_synthesis}, "

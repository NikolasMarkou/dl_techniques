...
class TrainingConfig:
    ...
    # Model (bias-free homogeneous Clifford U-Net)
    variant: str = "base"           # tiny | small | base
    ...
    # default from CLIFFORDUNET_CONFIGS. Primarily for the no-projection Gabor stem, where
    ...
    # Parameter-free per-level channel matching (zero-pad on increase, slice-up+add-skip on decrease) instead of 1x1 channel-adjust convs; bias-free, default OFF. See bfcliffordunet create_cliffordunet_denoiser.
    ...
    def __post_init__(self):
        if self.experiment_name is None:
            ...
            self.experiment_name = f"cliffordunet_denoiser_{self.variant}_{timestamp}"
...
class DenoisingVisualizationCallback(keras.callbacks.Callback):
    ...
    def _save_grid(self, epoch: int):
        ...
        fig.suptitle(
            f"CliffordUNet Denoiser - epoch {epoch} - same images, 3 noise regimes",
            fontsize=14, y=1.0,
        )
...
def train(config: TrainingConfig) -> keras.Model:
    ...
    # with an actionable message instead. The Clifford factory also raises
    # NotImplementedError on enable_deep_supervision=True; this guard fails even earlier.
    ...
    if config.init_from is not None:
        ...
        if len(report.loaded) == 0:
            raise ValueError(
                ...
                "mismatch? Ensure --variant/--patch-size/--cli-mode/--ctx-mode/gabor/"
...
def parse_arguments() -> argparse.Namespace:
    ...
    parser.add_argument("--mixed-precision", action="store_true",
                        help="enable mixed_float16 (fp16 compute on tensor cores, fp32 "
                             "weights + fp32 output). NOTE: the decoder's bilinear-upsample "
                             "grad forces XLA off, and XLA often outweighs fp16 here. Off by "
                             "default; may help at higher res / other GPUs / if the XLA "
                             "upsample-grad issue is fixed.")
    ...
    parser.add_argument(
        ...
        help="Warm-start model weights from a saved .keras checkpoint before training. "
             ...
             "denoiser. Architecture (variant/patch-size/cli-mode/ctx-mode/gabor/"

"""Training script for Conditional Bias-Free U-Net with CFG support."""
...
class ConditionalBFUNetDatasetBuilder(DatasetBuilder):
    ...
    def _create_multiscale_labels(
        ...
        labels = [tf.image.resize(clean_patch, dim) for dim in self.model_output_dims]
    ...
    def build(self) -> Tuple[tf.data.Dataset, tf.data.Dataset, Optional[int], Optional[int]]:
        ...
        train_files, train_labels = self._create_file_list_with_labels(
            self.config.train_image_dirs, self.config.max_train_files
        )
        val_files, val_labels = self._create_file_list_with_labels(
            self.config.val_image_dirs, self.config.max_val_files
        )
        ...
        if self.config.enable_deep_supervision and self.model_output_dims:
            train_ds = train_ds.map(
                self._create_multiscale_labels, num_parallel_calls=tf.data.AUTOTUNE
            )
        ...
        val_ds = val_ds.map(
            self._load_and_preprocess_image, num_parallel_calls=self.config.parallel_reads
        )
...
def conditional_sampling_with_cfg(
    ...
    CFG formula: s_guided(y, c) = s(y) + w * (s(y, c) - s(y))
    """
    if seed is not None:
...
class ConditionalSynthesisMonitorCallback(keras.callbacks.Callback):
        ...
        try:
            ...
            for cfg_scale in self.config.synthesis_cfg_scales:
                class_labels = np.repeat(
                    selected_classes, self.config.synthesis_samples_per_class
                )
    ...
    def _visualize_conditional_synthesis(
        ...
        try:
            ...
            fig.suptitle(f'Conditional Synthesis with CFG - Epoch {epoch}', fontsize=16)
...
class ConditionalDenoisingVisualizationCallback(keras.callbacks.Callback):
    ...
    def _save_comparison(self, epoch: int, noisy, clean, denoised, labels):
        try:
        ...
        if self.config.save_training_images and self.class_to_idx is not None:
            try:
                ...
                val_files, val_labels = builder._create_file_list_with_labels(
                    self.config.val_image_dirs, limit=100
                )
                callbacks.append(
                    ConditionalDenoisingVisualizationCallback(
                        self.config, val_files, val_labels,
                        self.experiment_dir, self.class_to_idx
                    )
                )
            except Exception as e:
    ...
    def run(
        ...
        if isinstance(dataset_builder, ConditionalBFUNetDatasetBuilder):
            temp_files, temp_labels = dataset_builder._create_file_list_with_labels(
                self.config.train_image_dirs, limit=10
            )
...
def main():
    ...
    config = ConditionalBFUNetConfig(
        train_image_dirs=['/path/to/train/data'],
        val_image_dirs=['/path/to/val/data'],
    ...
    logger.info(f"Conditional BFU-Net: variant={config.model_args['variant']}, "
                f"CFG={config.enable_cfg_training}, DS={config.enable_deep_supervision}, "

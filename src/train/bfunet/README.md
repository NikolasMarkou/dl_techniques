...
| `--block-normalization` | `layernorm` | Pre-activation normalization inside **every** ConvNeXt block. `layernorm` (default) = the original per-input `LayerNormalization` and is **byte-identical** to all existing checkpoints. `batchnorm` swaps in the new variance-only `BiasFreeBatchNorm` (no `moving_mean`, no `beta` — it divides by a *fixed* EMA-tracked running variance at inference), which **restores degree-1 homogeneity** `f(αx)=αf(x)` at inference. Per-input LayerNorm divides by a per-sample std that itself scales with the input, so it is only scale-*invariant* (degree-0); the fixed-stat BatchNorm is the degree-1 fix. **Caveat (homogeneity is architecture-wide):** the norm option only fixes the *norm* axis. A **fully-homogeneous** model additionally needs a homogeneous activation (`LeakyReLU`, the trainer default — pairs with `batchnorm`) **and** a homogeneous stem (the frozen **Gabor** stem, i.e. *not* `--no-gabor-stem`). `GELU` or the standard **GRN** stem stay non-homogeneous regardless of this flag. Homogeneity is an **inference-time** property (`training=False`); during training BatchNorm uses the per-batch variance and is scale-invariant. |

...
- model architecture with attention mechanisms
...
class MultiTaskMDNConfig:
    ...
    n_samples: int = 5000           # 5x more samples
...
class TimeSeriesGenerator:
    ...
    def _generate_gbm(self, drift: float, volatility: float, initial_price: float) -> np.ndarray:
        ...
        for t in range(1, n_steps):
            ...
            prices[t] = prices[t-1] * np.exp((drift - 0.5 * volatility**2) * dt + volatility * dW)
    ...
    def _generate_mean_reverting(self, theta: float, mu: float, sigma: float) -> np.ndarray:
        ...
        y = np.zeros(n_steps)
        y[0] = mu + np.random.normal(0, sigma)
...
class TaskScaler:
    """Scaler with better normalization for diverse patterns."""
    ...
    def fit(self, data: Dict[str, np.ndarray]):
        """Fit scalers for each task using robust statistics."""
        for task_name, task_data in data.items():
            ...
            robust_scaler.fit(task_data)
            self.robust_scalers[task_name] = robust_scaler

    def transform(self, task_name: str, data: np.ndarray, robust: bool = False) -> np.ndarray:
        """Transform data for a specific task."""
        if robust:
            return self.robust_scalers[task_name].transform(data)
        else:
            return self.scalers[task_name].transform(data)

    def inverse_transform(self, task_name: str, scaled_data: np.ndarray, robust: bool = False) -> np.ndarray:
        """Inverse transform data for a specific task."""
        if robust:
            return self.robust_scalers[task_name].inverse_transform(scaled_data)
        else:
...
def calibration_loss(y_true, y_pred, mdn_layer, temperature=None):
    ...
    # In practice, you might want a more sophisticated calibration loss
...
class MultiTaskTrainer:
    ...
    def prepare_data(self) -> Dict[str, Dict[str, Tuple[np.ndarray, np.ndarray, np.ndarray]]]:
        ...
        for task_id, task_name in enumerate(self.task_names):
            ...
            # Scale data (use robust scaling for more diverse patterns)
            train_scaled = self.task_scalers.transform(task_name, train_data, robust=True)
            val_scaled = self.task_scalers.transform(task_name, val_data, robust=True)
            test_scaled = self.task_scalers.transform(task_name, test_data, robust=True)
            ...
            logger.info(f"Task {task_name}: train={train_windows[0].shape[0]}, "
    ...
    def evaluate_model(self, model: MultiTaskMDNModel, task_data: Dict) -> Dict[str, TaskMetrics]:
        ...
        for task_id, (task_name, data) in enumerate(task_data.items()):
            ...
            # Inverse transform predictions (use robust scaling)
            point_estimates_orig = self.task_scalers.inverse_transform(task_name, point_estimates_np, robust=True)
            y_test_orig = self.task_scalers.inverse_transform(task_name, y_test, robust=True)

            # Scale variances (robust scaling)
            scale_factor = self.task_scalers.robust_scalers[task_name].scale_**2
            ...
            # Calculate confidence scores (simplified - based on prediction interval width)
            ...
            for bin_lower, bin_upper in zip(bin_lowers, bin_uppers):
                ...
                if in_bin.sum() > 0:
                    # Calculate accuracy in this bin (fraction of predictions that are covered)

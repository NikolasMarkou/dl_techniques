...
**Seven** classes interpolate a scalar over training progress and assign it somewhere. Not twelve: the "12" figure is not reconstructible under any definition tried. The reproducible base counts are **68** classes with a `*Callback` base across **51** files under `src/dl_techniques/` + `src/train/` (AST walk over `ClassDef.bases`), and **11** `.py` files under `src/dl_techniques/callbacks/` (10 plus `__init__.py`). Use the AST walk, not a grep: `grep -rn "^class .*Callback" src/dl_techniques/ src/train/ --include="*.py" | wc -l` also returns 68, but it is a **different** 68 — it counts `EarlyStoppingCallback` (`src/dl_techniques/models/ccnets/utils.py`, which has no base at all) and misses the one indented definition, `BenchmarkCallback` at `src/train/ntm/harness.py:754`. The agreement is a coincidence of two offsetting errors. (This paragraph used to say the grep gives 67; it does not, under any scope tried — corrected 2026-08-13.)
...
2. **2 of the 9 sites do not construct at all** on the as-written side (`bert/wikipedia/{pretrain,pretrain_english}.py` — `jit_compile=True`, see the bug note below), so the comparator's `CHK-CONSTRUCT` check reports MATERIAL there and no equivalence claim is even available.

...
- **Recorded but inert.** The field reaches a whole-config dump (`save_config_json` / `asdict` / `prepare_run_dir`) and nothing else. **Serialization is NOT consumption.** The worst of these were resnet's and vit's `save_model_checkpoints`: checkpoints ARE written, unconditionally, by `train.common.create_callbacks`, so setting the field `False` wrote `false` into `config.json` while checkpointing carried on. `save_best_only` has the identical shape — `common/callbacks.py` hard-codes `save_best_only=True`.

**The guard: `tests/test_train/test_config_fields_are_live.py`** (14 tests). For every config class in its `REGISTERED` list, every annotated field must have a consumption site. Add a row when you fix a config; never delete one.

Three design points, each forced by a measured failure:

1. **The read-set is scoped to the config object** (`self.X`, `config.X`, `cfg.X`, `*_config.X`, `<ClassName>.X`) — never a bare `<anything>.X`. Same lesson as the `save_dir` guard above, asserted directly by `test_the_read_set_rejects_an_unscoped_receiver` so the scoping cannot be loosened silently.
...
3. **`KNOWN_DEAD` pins rather than ignores.** Seven further dead fields found by the guard itself were outside the removal sweep's declared scope: `CIFARSOMConfig.perceptual_weight`, `CIFARSOMConfig.checkpoint_frequency`, `CopyTaskConfig.max_sequence_length`, `ExperimentConfig.csv_filename`, and `save_best_only` on both `TrainingConfig`s. `test_known_dead_fields_are_still_dead` FAILS if one of them becomes live, which is the signal to delete the exemption. An empty `KNOWN_DEAD` is the goal state — these are still bugs, merely unfixed ones.

RED-proven with ten mutations, each restored from `cp` backups verified by `sha256sum -c` (never `git checkout --`): M1-M8 re-add one removed field per touched config class, M9 adds an unrelated `unrelated_unused_knob` (isolating: the guard is not keyed to one field name), and all nine fire `test_config_declares_no_field_it_never_consumes[<class>]` naming the field. M10 WIRES an exempted field (`_ = config.save_best_only`) and fires the *other* assertion, `test_known_dead_fields_are_still_dead[train.vit.train_vit.py-TrainingConfig]` — so neither assertion is proven zero times.

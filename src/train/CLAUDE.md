...
**Gotcha — Keras 3.8 `.keras` + `by_name` is broken.** `model.load_weights(path.keras, by_name=True, skip_mismatch=True)` raises `ValueError("Invalid keyword arguments: {'by_name': True}")` in Keras 3.8+ — the `by_name` path is only supported for legacy `.h5`/`.hdf5` files. Use `load_weights_from_checkpoint` (full-model load + layer-by-layer `set_weights`) for name-based transfer from `.keras` checkpoints. Re-measured 2026-08-10 (`grep -rn "by_name=by_name" src/dl_techniques/`): **six** `load_pretrained_weights` helpers still forward the flag straight into `self.load_weights` and carry this latent bug — `models/vision/convnext/convnext_v1.py`, `models/vision/resnet/model.py`, `models/language/distilbert/model.py`, `models/language/modern_bert/model.py`, `models/language/bert/model.py`, `models/language/fnet/model.py`. The three previously listed here (`cliffordnet/model.py`, `bias_free_denoisers/bfunet.py`, `convnext/convnext_v2.py`) have since been migrated to `load_weights_from_checkpoint` and now ignore the argument — do not use them as evidence of the bug.
...
**This section is a DIFFERENT axis from the "Scripts That Don't Use `train.common` Callbacks" table above.** That table is about adoption of the generic `create_callbacks()` scaffold; none of its four packages (`bert/wikipedia`, `blt`, `yolo12/train_multitask`, `tabm`) appears anywhere in the six-class schedule family below. Do not merge the two lists.
...
| `TeacherEMACallback` | `src/dl_techniques/models/vision/depth_anything/teacher_ema.py` |
...
Separately, **14 bare one-liners** are excluded as a class: every one of them passes only `learning_rate=`, by keyword. **There is no longer a positional-LR site anywhere under `src/train/`** — the sole one, `convunext/train_convunext.py:229`, went away with `src/train/convunext/`, deleted on 2026-08-14 when `models/vision/convunext` and `bfconvunext` merged onto one builder. Two corrections in one, so read the digits carefully: the old figure was **16 = 15 keyword + 1 positional**, and the positional site is the only part the deletion removed; the keyword half was **already 14, not 15**, when that sentence was written (re-measured at the pre-merge commit `6b2f5a4bb`, which still contained the deleted directory). Re-derive with an AST scan, not a grep — `learning_rate=` appears in longer argument lists too: parse `src/train/**/*.py`, keep `ast.Call` nodes whose `ast.unparse(func)` starts with `keras.optimizers.` and whose attribute is one of `Adam`/`AdamW`/`SGD`/`RMSprop`, and count those with zero positional args and `[k.arg for k in keywords] == ['learning_rate']` → **14** (`blt` ×2, `convnext/train_convnext_v2_mae` ×2, `ideogram4`, `kan`, `sam`, `sam2`, `sd3_mmdit`, `som_nd_soft/train_mnist`, `tabm`, `vae` ×3). The same scan reports **59** `keras.optimizers.*` calls in total, against the **63** claimed in table (c) above — that gap is PRE-EXISTING (60 by the same scan at `6b2f5a4bb`, i.e. only 1 of the 4 is this deletion) and is left uncorrected here rather than re-derived under a filter this note cannot reconstruct. Routing any of these through the builder replaces one line with a dict-literal plus a call — it is net line-**positive**. "Consolidate all 63 optimizer sites" is not a supportable framing.
...
- **`DeepSupervisionWeightScheduler`** (`src/train/resnet/train_resnet.py`) had **zero** test coverage until 2026-08-24, and the gap had already cost something: the callback was a **silent no-op for its whole existence**. `on_epoch_begin` assigned `self.model.loss_weights = new_weights`, and Keras 3 `Model` has no such attribute (reading it raises `AttributeError`) while `CompileLoss` snapshots the weights into `_flat_losses` at build time — so the deep-supervision weight SCHEDULE never reached the loss and the compile-time `1/num_outputs` weights were used for every epoch. The weights are now non-trainable `keras.Variable`s produced by `build_compile_spec`, which `CompileLoss.call`'s `ops.multiply(value, loss_weight)` reads live; see the two `D-020` anchors in `train_resnet.py`. Covered by `tests/test_train/test_resnet/test_the_trainer_compiles_and_the_schedule_moves_the_loss.py`, whose deciding arm freezes the optimizer at `learning_rate=0.0` so the ONLY thing that can move the reported loss between epochs is the weight vector (measured: `linear_low_to_high` drift **5.535e-01**, `constant_equal` drift **exactly 0.0**). An "it ran without raising" assertion is structurally incapable of seeing this defect — that is how it survived.
...
Section (g) above rules on the *trainer* configs. This subsection records the **model-package** refusals measured by the same plan, so the next factory-adoption sweep over `src/dl_techniques/models/{gpt2,wave_field}/` reads them instead of re-deriving them. Each refusal below cost a measurement; none of them is a style opinion. The model-side copies live in `src/dl_techniques/models/language/gpt2/README.md` § 15 and `src/dl_techniques/models/language/wave_field/README.md` § 15.
...
**4. `models/language/gpt2/` has ZERO adoptable factory sites — and the obvious grep is a GHOST.** `gpt2.py` builds exactly two things: a `TextDecoder` (which is itself already composed of `TransformerLayer`, `create_normalization_layer`, and `dl_techniques.utils.masking`'s mask builders) and, only when `tie_word_embeddings=False`, a bare `Dense(vocab_size, use_bias=False)` LM head. No factory covers a vocabulary projection — it is not an attention / FFN / normalization / embedding / activation primitive. **`grep -rn "create_.*_layer" src/dl_techniques/models/language/gpt2/ --include="*.py"` returns 0 hits, and that measures the wrong thing**: the model reaches the factories correctly THROUGH `TextDecoder`, one level down, which is the right depth of reuse per `src/dl_techniques/models/CLAUDE.md` § Layer Reuse Policy. Do not "fix" a zero here.
...
**A grep-scoping trap, for the third time in this file** (see table (c) and the closing note of § (g)): any `src/train/`-wide grep quoted in prose must carry `--include="*.py"` or a `-- "*.py"` pathspec, or this document's own text becomes the extra hit. Item 4's grep hit it live: the moment `src/dl_techniques/models/language/gpt2/README.md` was written, an unscoped `grep -rn "create_.*_layer" src/dl_techniques/models/language/gpt2/` returned **3** hits — all of them prose in the new README explaining that the answer is zero. With `--include="*.py"` it is 0, as claimed.

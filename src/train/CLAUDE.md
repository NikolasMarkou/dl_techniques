...
### (g) The CLM `TrainingConfig` ruling — base class ADOPTED for two trainers, REFUSED for `train_memory.py`
...
**REFUSED for `train/wave_field/train_memory.py`, and rule (i) makes the refusal mechanical rather than a judgement call.**

```
grep -n "num_layers\|num_heads" src/train/wave_field/train_memory.py   # → 0 hits
```

The base carries `num_layers` and `num_heads`; that config does not have them and nothing in the file reads them. Inheriting would **ADD two fields no code reads and no CLI flag sets** — the dead-knob class `tests/test_train/test_config_fields_are_live.py` exists to catch, and the "invent config toggles for code that does not exist" anti-pattern sections (b) and the tree_transformer non-adoption already refuse. It also overrides `learning_rate` 3e-4 → 1e-5 and adds eight fields of its own (`field_size`, `top_k`, `memory_lr`, `phase1_steps`, `phase2_steps`, `phase3_steps`, `warmup_num_batches`, `init_from`). It is the feature fork §2 of that plan's findings already classed it as, and the config measurement agrees independently. **Do not re-litigate this by "just adding the two fields".**

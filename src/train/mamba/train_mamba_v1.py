r"""Pretrain Mamba-1 on Wikipedia through stock ``fit()``.

Usage::

    # what the CLI offers -- allocates nothing, touches no GPU
    MPLBACKEND=Agg .venv/bin/python -m train.mamba.train_mamba_v1 --help

    # a tiny smoke run on the already-staged Wikipedia dump
    MPLBACKEND=Agg CUDA_VISIBLE_DEVICES=1 .venv/bin/python -m train.mamba.train_mamba_v1 \
...
:mod:`train.mamba.common_v1`, so there is exactly one place to read and
exactly one place a defect can hide.
...
THE FILE IS ``train_mamba_v1.py``, NEVER ``train.py``.
...
SEEDING IS NOT REPEATED HERE.
    ``common_v1.train`` calls ``set_seeds(config.seed)`` as its first
    statement.

``--gpu`` IS NOT A CONFIG FIELD.
    It acts on the process, is consumed by ``setup_gpu`` and never reaches
    :class:`~train.mamba.common_v1.MambaV1TrainingConfig`.
...
PROGRAM_NAME: str = "train_mamba_v1.py"
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        prog=PROGRAM_NAME,
        description=(
            "Pretrain Mamba-1 (selective-scan SSM) on the staged Wikipedia "
            "dump with stock Keras fit(). Tokenizer is Tiktoken. "
...
def parse_arguments(
...
) -> Tuple[argparse.Namespace, MambaV1TrainingConfig]:
    ...
    :rtype: Tuple[argparse.Namespace, MambaV1TrainingConfig]
    :raises SystemExit: As argparse does, on ``--help`` or a bad flag.
    :raises ValueError: From ``MambaV1TrainingConfig.__post_init__`` on a
...
def main(argv: Optional[Sequence[str]] = None) -> None:
    ...
    logger.info(
        "Mamba (v1): training variant %r (of %s) on %s at seq_len=%d, batch=%d",
        config.variant,
        list(variant_names()),
        config.dataset_root,
    ...
    logger.info(
        "Mamba (v1) %s training complete; artifacts in %s",

...
from dl_techniques.models.clip.model import CLIP, create_clip_model, create_clip_variant
...
class CLIPAccuracy(keras.metrics.Metric):
    ...
    Args:
        direction: Either 'i2t' (image-to-text) or 't2i' (text-to-image).
    ...
    def __init__(
        ...
        **kwargs: Any
        ...
        if direction not in ['i2t', 't2i']:
            raise ValueError(f"direction must be 'i2t' or 't2i', got {direction}")
        ...
        logger.info(f"Initialized CLIPAccuracy metric for {direction} direction")
    ...
    def update_state(
        ...
        if self.direction == 'i2t':
            logits = y_pred['logits_per_image']  # (batch_size, batch_size)
        else:
            logits = y_pred['logits_per_text']  # (batch_size, batch_size)

        # Predictions: argmax along the last axis
        predictions = ops.argmax(logits, axis=-1)  # (batch_size,)

        # Ground truth: diagonal indices [0, 1, 2, ..., batch_size-1]
        batch_size = ops.shape(logits)[0]
        labels = ops.arange(batch_size, dtype='int32')

        # Count correct predictions
        correct = ops.sum(ops.cast(predictions == labels, 'float32'))
...
class CLIPRecallAtK(keras.metrics.Metric):
    ...
    top-K predictions. This is a more lenient metric than accuracy and is
    commonly used for retrieval tasks.
    ...
    def __init__(
        ...
        **kwargs: Any
        ...
        logger.info(f"Initialized CLIPRecallAtK metric: k={k}, direction={direction}")
    ...
    def update_state(
        ...
        # Use negative logits for top_k to get highest values
        top_k_indices = ops.top_k(-logits, k=ops.minimum(self.k, batch_size))[1]
        # Shape: (batch_size, k)

        # Ground truth: diagonal indices [0, 1, 2, ..., batch_size-1]
        labels = ops.arange(batch_size, dtype='int32')  # (batch_size,)
        labels_expanded = ops.expand_dims(labels, axis=-1)  # (batch_size, 1)

        # Check if correct label is in top-k predictions
        # Broadcasting: (batch_size, 1) == (batch_size, k) -> (batch_size, k)
    ...
    def result(self) -> keras.KerasTensor:
        """Compute recall@k as percentage of correct retrievals in top-k."""
        return self.correct / ops.maximum(self.total, 1e-7)
...
def create_training_example():
    """
    Complete example of training a CLIP model.

    This example demonstrates:
    1. Creating a CLIP model
    2. Setting up data loaders
    3. Configuring optimizer and learning rate schedule
    4. Creating metrics and callbacks
    5. Training the model
    6. Saving checkpoints and history
    """
    ...
    # 1. Create CLIP model
    # For production, use: model = create_clip_variant("ViT-B/32")
    logger.info("Creating CLIP model...")

    # Using imports from project (commented for this example)
    ...
    # For this example, assume model is created
    logger.info("Model created (placeholder)")
    ...
    logger.info("Optimizer and learning rate schedule created")

    # 4. Create metrics
    metrics = [
        CLIPAccuracy(direction='i2t', name='i2t_accuracy'),
        CLIPAccuracy(direction='t2i', name='t2i_accuracy'),
        CLIPRecallAtK(k=5, direction='i2t', name='i2t_recall@5'),
        CLIPRecallAtK(k=5, direction='t2i', name='t2i_recall@5'),
    ]

    logger.info(f"Created {len(metrics)} metrics")

    # 5. Create data loader and datasets
    data_loader = CLIPDataLoader(
        image_size=image_size,
        context_length=context_length,
        augmentation=True
    ...
    num_samples = 100
    dummy_images = [f"image_{i}.jpg" for i in range(num_samples)]
    dummy_captions = [f"caption {i}" for i in range(num_samples)]

    logger.info("Data loader created (using dummy data for demo)")
    ...
    trainer.save_model('clip_final.keras')
...
def create_inference_example():
    """
    Complete example of using a trained CLIP model for inference.

    This example demonstrates:
    1. Loading a trained model
    2. Encoding images and texts
    3. Computing similarities
    4. Performing retrieval tasks
    """
    logger.info("=== CLIP Inference Example ===")

    # 1. Load trained model
    # model = keras.models.load_model('clip_model.keras')
    logger.info("Model loading (placeholder)")

    # 2. Create data loader and inference engine
    data_loader = CLIPDataLoader(
        image_size=224,
        context_length=77
    )
    ...
    # 3. Example: Image-to-text retrieval
    logger.info("\nExample: Retrieving captions for an image")
    ...
    results = inference.retrieve_texts(
    ...
    # 4. Example: Text-to-image retrieval
    logger.info("\nExample: Retrieving images for a text query")
...
if __name__ == "__main__":
    ...
    Run this script to see example usage of the CLIP training

r"""Pretrain Gemma3 on Wikipedia through stock ``fit()``.

Usage::

    # what the CLI offers -- allocates nothing, touches no GPU
    MPLBACKEND=Agg .venv/bin/python -m train.gemma.train_gemma --help

    # a tiny smoke run on the already-staged Wikipedia dump
    MPLBACKEND=Agg CUDA_VISIBLE_DEVICES=1 .venv/bin/python -m train.gemma.train_gemma \
        --variant tiny --hidden-size 32 --num-layers 2 --num-attention-heads 4 \
        --num-key-value-heads 2 --ffn-hidden-size 64 \
...
callbacks and ``fit()`` itself all live in :mod:`train.gemma.common`, so
...
``--gpu`` IS NOT A CONFIG FIELD.
    ...
    :class:`~train.gemma.common.Gemma3TrainingConfig`. :data:`NON_CONFIG_DESTS`
...
from train.gemma.common import (
...
PROGRAM_NAME: str = "train_gemma.py"
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        prog=PROGRAM_NAME,
        description=(
            "Pretrain Gemma3 (sandwich-normed, mixed sliding-window/full "
            "attention decoder) on the staged Wikipedia dump with stock "
            f"Keras fit(). Tokenizer is Tiktoken. Variants: {list(variant_names())}."
        ),
    )
...
def parse_arguments(
...
) -> Tuple[argparse.Namespace, Gemma3TrainingConfig]:
    ...
    :rtype: Tuple[argparse.Namespace, Gemma3TrainingConfig]
    :raises SystemExit: As argparse does, on ``--help`` or a bad flag.
    :raises ValueError: From ``Gemma3TrainingConfig.__post_init__`` on a
...
def main(argv: Optional[Sequence[str]] = None) -> None:
    ...
    logger.info(
        "Gemma3: training variant %r (of %s) on %s at seq_len=%d, batch=%d",
        config.variant,
        list(variant_names()),
        config.dataset_root,
    ...
    logger.info(
        "Gemma3 %s training complete; artifacts in %s",

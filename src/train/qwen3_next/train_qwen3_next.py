r"""Pretrain Qwen3Next on Wikipedia through stock ``fit()``.

**Scope: `Qwen3Next` ONLY.** See ``train.qwen3_next.common``'s module
docstring for why this is a new sibling package rather than an addition to
``src/train/qwen/`` (that package's own D-001 anchor forbids importing
``qwen3_next.py`` from it).

Usage::

    # what the CLI offers -- allocates nothing, touches no GPU
    MPLBACKEND=Agg .venv/bin/python -m train.qwen3_next.train_qwen3_next --help

    # a tiny smoke run on the already-staged Wikipedia dump, MoE-enabled
    # "tiny" variant (the default)
    MPLBACKEND=Agg CUDA_VISIBLE_DEVICES=1 .venv/bin/python -m \
        train.qwen3_next.train_qwen3_next \
        --variant tiny --hidden-size 32 --num-layers 1 --num-attention-heads 4 \
...
callbacks and ``fit()`` itself all live in :mod:`train.qwen3_next.common`,
so there is exactly one place to read and exactly one place a defect can
hide.
...
THE FILE IS ``train_qwen3_next.py``, NEVER ``train.py``.
...
``--gpu`` IS NOT A CONFIG FIELD.
    ...
    :class:`~train.qwen3_next.common.Qwen3NextTrainingConfig`.
...
PROGRAM_NAME: str = "train_qwen3_next.py"
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        prog=PROGRAM_NAME,
        description=(
            "Pretrain Qwen3Next (hybrid linear-attention + gated-attention "
            "decoder, interleaved MoE blocks on every variant except "
            "'80b') on the staged Wikipedia dump with stock Keras fit(). "
            f"Tokenizer is Tiktoken. Variants: {list(variant_names())}."
        ),
    )
...
def parse_arguments(
...
) -> Tuple[argparse.Namespace, Qwen3NextTrainingConfig]:
    ...
    :rtype: Tuple[argparse.Namespace, Qwen3NextTrainingConfig]
    :raises SystemExit: As argparse does, on ``--help`` or a bad flag.
    :raises ValueError: From ``Qwen3NextTrainingConfig.__post_init__`` on a
...
def main(argv: Optional[Sequence[str]] = None) -> None:
    ...
    logger.info(
        "Qwen3Next: training variant %r (of %s) on %s at seq_len=%d, batch=%d",
        config.variant,
        list(variant_names()),
        config.dataset_root,
    ...
    logger.info(
        "Qwen3Next %s training complete; artifacts in %s",

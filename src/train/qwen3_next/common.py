"""Shared building blocks for the Qwen3Next (hybrid linear-attention MoE) trainer.

**Scope: `Qwen3Next` ONLY.** `src/train/qwen/common.py` hosts plain `Qwen3`
and its own D-001 anchor forbids that module from importing `qwen3_next.py`
or `qwen3_embeddings.py` (`plans/plan-2026-09-12T173329-e20362c4/decisions.md`
D-001) -- so `Qwen3Next` gets a NEW sibling package rather than colocation
(`plans/plan-2026-09-13T073704-245ab5d5/decisions.md` D-003). This module
imports ONLY from `qwen3_next.py`.

`Qwen3Next.call()` (`dl_techniques.models.language.qwen.qwen3_next.Qwen3Next`)
is causal-LM shaped exactly like plain `Qwen3`: it bakes its own untied
`lm_head` and returns a PLAIN TENSOR of logits, shape
`(batch, seq_len, vocab_size)`, with the default `return_dict=False`
(confirmed by direct read, `qwen3_next.py:391-444`). So this module follows
`src/train/qwen/common.py`'s Pattern-3 scaffold verbatim
(`findings/qwen3next-embedding-reranker-requirements.md` Thread 1) --
config dataclass + argparse + `config_from_args` single-wiring-site +
`build_datasets`/`build_optimizer`/`build_model`/`train` -- with exactly one
substantive difference in `build_model`: `CausalLanguageModel(...,
aggregate_backbone_losses=True)`.

**Why `aggregate_backbone_losses=True`.** Every shipped `Qwen3Next` variant
except `"80b"` sets `num_experts > 1` (`tiny`: 4, `small`: 8, `80b_a3b`: 512;
`"80b"`: 1, dense). Whenever `num_experts > 1`,
`MixtureOfExperts.call()` (`layers/moe/layer.py:275-290`) calls
`self.add_loss(aux_loss)` (load-balancing) and `self.add_loss(z_loss)`
(router-logit regularizer) during `training=True`, for every block -- this
is EXACTLY the shape `aggregate_backbone_losses=True` exists for, already
proven non-decorative for HNet's boundary-ratio term
(`plans/plan-2026-09-13T073704-245ab5d5/decisions.md`, Step 2). Without the
flag, those `self.add_loss` contributions are computed but never folded into
`CausalLanguageModel`'s own reported/optimized total loss (`clm.py:33-42,
261-273, 786-787, 819-820`).

**Default variant is `"tiny"` (MoE-enabled, smallest), never `"80b"`/
`"80b_a3b"`.** `"80b"` is DENSE (`num_experts=1`) -- an aggregation RED-proof
against it would be vacuous (`model.backbone.losses` empty, nothing to
aggregate). `"80b_a3b"` IS MoE-enabled but is the full released
Qwen3-Next-80B-A3B config (`qwen3_next.py`'s own D-112/D-204 anchor: "do not
shrink for feasibility") -- production-scale, wrong for a smoke default.
`"tiny"` (4 experts, 1 active/tok, hidden_size=512, 3 blocks) is MoE-enabled
AND small, so it is both a correct and a fast default/smoke choice.

No MoE-specific CLI overrides (`--num-experts`/`--num-experts-per-tok`/
`--moe-intermediate-size`) are exposed here, mirroring plain Qwen3's own
choice not to expose `moe_layers`/`num_experts`
(`src/train/qwen/common.py:50-58`) -- unlike plain Qwen3's `moe_layers`,
which is a list of LAYER INDICES that a `--num-layers` override could put
out of range, `Qwen3Next` applies MoE uniformly to every block whenever
`num_experts > 1`, so there is no equivalent index-range hazard to guard
against here either way; adding those knobs is a future, independently
dispatchable enhancement, not something `--hidden-size`/`--num-layers`
overrides require.

One thing this module deliberately does NOT do, for the same reason
`qwen/common.py`/`zamba2/common.py` give:

* **No `ClmPretrainConfig`/`load_train_val_datasets` reuse.** That wrapper
  wraps every label tensor as `{"logits": y}` for its dict-output callers.
  `Qwen3Next.call` returns a PLAIN TENSOR, not a dict, so this module calls
  the leaf helpers directly (`load_wikipedia_train_val`,
  `preprocess_clm_packed_dataset`, `estimate_clm_steps_per_epoch`), the same
  as `qwen`/`zamba2`/`mamba`/`gemma`'s `common.py`.

Public surface:
    * :data:`VARIANT_NAMES` -- the shipped :data:`Qwen3Next.MODEL_VARIANTS`
      keys (``"80b_a3b"``, ``"80b"``, ``"small"``, ``"tiny"``).
    * :class:`Qwen3NextTrainingConfig` -- the run knobs. Every field is
    ...
    * :func:`build_datasets` -- the Wikipedia packed-CLM pipeline (identical
      shape to qwen's/zamba2's/mamba's/gemma's).
    * :func:`build_optimizer` / :func:`build_model` -- AdamW through
      ``optimizer_builder``; the model is a ``CausalLanguageModel`` wrapping
      ``Qwen3Next.from_variant`` with ``loss_fn=create_clm_loss_fn(config)``
      and ``aggregate_backbone_losses=True``.
...
RESULTS_DIR_PREFIX: str = "qwen3_next"
...
``train.qwen.common``'s convention: Keras spells LayerNorm/RMSNorm
...
"""Default ``--variant`` -- the smallest shipped size AND MoE-enabled
(``num_experts=4``), never ``"80b"`` (dense) or ``"80b_a3b"`` (production
scale). See the module docstring."""

VARIANT_NAMES: Tuple[str, ...] = tuple(Qwen3Next.MODEL_VARIANTS)
"""Every ``--variant`` choice: the four :data:`Qwen3Next.MODEL_VARIANTS`
keys (``80b_a3b``, ``80b``, ``small``, ``tiny``), in declaration order."""
...
@dataclass
class Qwen3NextTrainingConfig:
    """Knobs for one Qwen3Next pretraining run.
    ...
    :param max_seq_length: Tokens the MODEL sees per window. The packer is
        ...
        token to spend. Also forwarded as ``Qwen3Next``'s ``max_seq_len``
        (the RoPE table horizon inside the gated-attention sublayers).
    ...
    :param hidden_size: Overrides the variant's hidden width. ``None`` keeps
        ...
        (``Qwen3Next.__init__`` raises ``ValueError`` otherwise) -- this
        config does not re-validate that interaction, since it does not know
        the variant's other knobs; the model constructor is the single
        source of truth for it.
    :param num_layers: Overrides the variant's block count. Unlike plain
        ``Qwen3``'s ``moe_layers`` (a list of layer INDICES that an
        out-of-range override could invalidate), ``Qwen3Next`` applies MoE
        uniformly to every block whenever ``num_experts > 1`` -- so no
        companion override is needed here.
    ...
    def variant_overrides(self) -> Dict[str, Any]:
        """:returns: The non-``None`` architecture overrides
            ...
            ``num_key_value_heads``) as a kwargs dict for
            :meth:`Qwen3Next.from_variant`. Unlike ``Qwen3TrainingConfig``'s
            same-named property, no companion ``moe_layers`` reset is
            needed: ``Qwen3Next`` applies MoE uniformly to every block
            whenever ``num_experts > 1``, so a ``num_layers`` override
            cannot put any per-layer index out of range.
        :rtype: Dict[str, Any]
        """
...
def add_common_arguments(parser: argparse.ArgumentParser) -> argparse.ArgumentParser:
    """Register the shared Qwen3Next training flags.
    ...
    parser.add_argument(
        ...
        help="Qwen3Next model size to train.",
    )
    parser.add_argument(
...
def config_from_args(args: argparse.Namespace) -> Qwen3NextTrainingConfig:
    """Build a config from a parsed namespace.

    The ONE wiring site between :func:`add_common_arguments` and
    :class:`Qwen3NextTrainingConfig`. A flag that does not arrive here
    silently does nothing.

    :param args: A namespace produced by a parser carrying the common flags.
    :type args: argparse.Namespace
    :returns: The config.
    :rtype: Qwen3NextTrainingConfig
    """
    return Qwen3NextTrainingConfig(
...
def build_datasets(
    ...
    Identical shape to ``train.qwen.common.build_datasets`` /
    ``train.zamba2.common.build_datasets`` / ``train.mamba.common.build_datasets``.

    :param config: The run config.
    :type config: Qwen3NextTrainingConfig
    ...
    logger.info(
        f"Qwen3Next corpus: {n_train} train / {n_val} val articles; "
...
def build_model(
    ...
    """Create and compile the Qwen3Next causal-LM model for one run.

    ``Qwen3Next`` already bakes its own LM head and returns a plain logits
    tensor (see the module docstring), so ``Qwen3Next.from_variant`` is
    constructed exactly as before and then wrapped in
    ``CausalLanguageModel``: ``skip_head=True`` (the backbone's own output IS
    the logits, no head/weight-tying is built), ``pre_shifted=True`` (the
    packed-CLM pipeline already yields shifted ``(x, y)`` pairs, so no
    internal shift is applied), ``loss_fn=create_clm_loss_fn(config)``
    (preserves ``--loss-type focal``/``--label-smoothing``), and
    ``aggregate_backbone_losses=True`` -- the one substantive difference from
    plain Qwen3's ``build_model``: MoE-enabled variants (every one except
    ``"80b"``) call ``self.add_loss`` for load-balancing/z-loss per block,
    and this flag folds those contributions into the reported/optimized
    total loss instead of silently discarding them. ``compile()`` needs no
    ...
    """
    qwen3_next_model = Qwen3Next.from_variant(
        config.variant,
...
def train(config: Qwen3NextTrainingConfig) -> Tuple[CausalLanguageModel, Any, str]:
    """Pretrain Qwen3Next on Wikipedia with stock ``fit()``.
    ...
    # qwen/common.py's / zamba2/common.py's / mamba/common.py's `train`.

"""BERT Fine-tuning Script for Sentiment Analysis with Tiktoken.
...
class FinetuneConfig:
    ...
    full_analysis_dir: str = (
        "results/bert_sentiment_finetune/full_analysis"
    )
...
def create_sentiment_model(
    ...
    This function loads the pretrained BERT encoder, attaches a
    ...
    try:
        ...
        logger.error(
            f"Failed to load BERT encoder. Ensure the path is correct. "
            f"Error: {e}"
        )
        raise
...
def finetune_sentiment_model(
    ...
    """
    logger.info("=" * 80)
    logger.info("BERT Sentiment Analysis Fine-tuning with Tiktoken")
    logger.info("=" * 80)
    ...
    final_model_path = os.path.join(
        config.save_dir,
        "bert_sentiment_final_best.keras"
    )

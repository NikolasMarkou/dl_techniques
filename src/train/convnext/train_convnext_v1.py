"""Train ConvNeXt V1 on MNIST / CIFAR-10 / CIFAR-100.

A thin wrapper: everything (CLI, data, schedule, callbacks, artifacts) lives in
``train.convnext.common``, shared with the ConvNeXt V2 trainer. This file only
fixes the model family. ``--variant`` offers exactly the keys of
``ConvNeXtV1.MODEL_VARIANTS``.
...
def main(argv: Optional[Sequence[str]] = None) -> None:
    """Parse ``argv`` (``None`` reads ``sys.argv[1:]``) and train ConvNeXt V1."""
    run_convnext("v1", argv)

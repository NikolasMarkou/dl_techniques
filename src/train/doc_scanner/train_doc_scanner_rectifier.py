r"""Train DocScanner's RECTIFICATION module (stage 2) through stock ``fit()``.
...
this script trains ONLY the 12-iteration progressive rectifier, against the
``[f_gt, g]`` flow stack, with the paper's own recipe: the Eq. 9-14 sequence
loss, AdamW, batch 12, peak learning rate 1e-4 reached after a warmup that is
4.8% of the run. Its twin is ``train_doc_scanner_segmenter.py``.

THREE HYPERPARAMETERS OF THIS RECIPE ARE THIS REPO'S CHOICE, not a
reproduction: the warmup CURVE (linear), the weight decay (1e-4) and the
gradient clip (global norm 1.0). The paper states none of them. See
:mod:`train.doc_scanner.common` and ``README.md``.
...
def build_parser() -> argparse.ArgumentParser:
    """Build the real parser: the common flags at the rectifier's defaults.
    ...
    parser = argparse.ArgumentParser(
        prog=PROGRAM_NAME,
        description=(
            "Train DocScanner's rectification module (the 12-iteration "
            "progressive refiner) with stock Keras fit(). Stage 2 of two; the "
            "segmenter trains separately with `python -m "
            "train.doc_scanner.train_doc_scanner_segmenter`."
        ),
    )
...
def main(argv: Optional[Sequence[str]] = None) -> None:
    ...
    logger.info(
        "DocScanner rectifier: training variant %r on the %r corpus",
        config.model_variant, config.data_source,
    )
    _, _, results_dir = train(config)
    logger.info(
        "DocScanner rectifier training complete; artifacts in %s", results_dir
    )

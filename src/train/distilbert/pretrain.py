"""DistilBERT Pre-training Script with Masked Language Modeling.

Pre-trains a DistilBERT encoder using MLM on a text dataset (IMDB reviews by
default). Saves both the full MLM model and the encoder separately for
downstream fine-tuning.

Cloned from ``train.bert.pretrain``'s Pattern-3 MLM shape (same
``MaskedLanguageModel`` wrapper, same ``train.common.nlp`` helpers). No
teacher-student distillation loss: ``DistilBERT`` (see
``dl_techniques.models.language.distilbert.model``) is a plain foundation
...
class DistilBertTrainingConfig:
    """Configuration for DistilBERT MLM pre-training."""

    # Model
    distilbert_variant: str = "tiny"
    vocab_size: int = 100277  # Tiktoken cl100k_base
    max_seq_length: int = 128

    # Tokenizer (Tiktoken cl100k_base) -- DistilBERT has no token-type
    # embeddings and no pooler, but the tokenizer still needs CLS/SEP/PAD/MASK
    # ids to decide which positions are eligible for masking (special_token_ids
    # below), same convention as bert/fnet.
    ...
    # Paths
    save_dir: str = "results/distilbert_pretrain"
...
def create_distilbert_mlm_model(config: DistilBertTrainingConfig) -> MaskedLanguageModel:
    """Create DistilBERT encoder wrapped in MaskedLanguageModel."""
    logger.info(f"Creating DistilBERT-{config.distilbert_variant.upper()} encoder...")
    encoder = DistilBERT.from_variant(
        variant=config.distilbert_variant,
    ...
    # Build to count parameters. DistilBERT has no token_type_ids input
    # (unlike BERT), so the dummy batch is input_ids + attention_mask only.
...
def compile_model(mlm_model: MaskedLanguageModel, config: DistilBertTrainingConfig, steps_per_epoch: int):
    """Compile MLM model with AdamW and warmup schedule.

    Raw ``keras.optimizers.AdamW`` (no ``jit_compile``), matching
    ``train.bert.pretrain.compile_model``'s own convention exactly --
...
def train_distilbert_mlm(config: DistilBertTrainingConfig) -> Tuple[MaskedLanguageModel, keras.callbacks.History]:
    """Run DistilBERT MLM pre-training."""
    logger.info("=" * 60)
    logger.info("DistilBERT MLM Pre-training with Tiktoken")
    logger.info("=" * 60)
    ...
    mlm_model = create_distilbert_mlm_model(config)
    compile_model(mlm_model, config, steps_per_epoch)
    callbacks, results_dir = create_nlp_callbacks(
        model_name=f"DistilBERT-{config.distilbert_variant}",
        results_dir_prefix="distilbert_pretrain",
    ...
    # `train.bert.pretrain` (the file this script clones) does not write a
    # config.json at all; the zamba2/gemma/qwen CLM trainers earlier in this
    # plan do (`save_config_json(config, results_dir, "config.json")`), and
    # this plan's own Success Criteria #2 requires it of every new trainer
    ...
    final_path = os.path.join(config.save_dir, "distilbert_mlm_final_best.keras")
    mlm_model.save(final_path)
    encoder_path = os.path.join(config.save_dir, "pretrained_distilbert_encoder_best.keras")
...
# own convention for the same shared helper (D-010 in the bert history).
...
def parse_arguments(argv: Optional[list] = None) -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description="DistilBERT MLM Pre-training")
    parser.add_argument('--gpu', type=int, default=None, help='GPU device index')
    parser.add_argument('--variant', type=str, default='tiny', help='DistilBERT variant')
...
def main(argv: Optional[list] = None) -> None:
    """Main entry point for DistilBERT MLM pre-training."""
    args = parse_arguments(argv)

    setup_gpu(gpu_id=args.gpu)

    config = DistilBertTrainingConfig()
    config.distilbert_variant = args.variant
    config.num_epochs = args.epochs
    config.batch_size = args.batch_size
    config.max_samples = args.max_samples

    logger.info(f"Config: variant={config.distilbert_variant}, epochs={config.num_epochs}, "
                f"batch_size={config.batch_size}, lr={config.learning_rate}, "
                f"max_samples={config.max_samples}")

    mlm_model, history = train_distilbert_mlm(config)
    ...
    logger.info(f"Pre-training complete! Encoder: {config.save_dir}/pretrained_distilbert_encoder_best.keras")

...
- **A reused `--experiment-name` reuses the directory** (`mkdir(exist_ok=True)`) and
  overwrites its files. Pass a fresh name for every run you want to keep. The default name
  is `powermlp_<dataset>_<architecture>_<timestamp>`.
- One GPU job at a time. A foreign process on the same GPU shows up as an out-of-memory
  error at import.
- `--gpu N` selects device `N` (default: all visible GPUs).
...
| `--gpu` | all visible GPUs | GPU device index. |
...
- **`training_dashboard.png`**: six panels: Loss (log axis, with the epoch-0 baseline star,
  clipped and annotated only if it is more than 10x the curves), Accuracy, Learning rate,
...
- **Dashboard cadence**: redrawn after epoch 1, then every `max(1, planned_epochs // 20)`
  ...
  100-epoch run draws 21 times, not 100. A render costs about 1.0 to 1.2 s (measured, CPU)
...
`EarlyStopping` restores the epoch-22 weights in each of them. Details of the final run:
...
| on | 0.4345 | 1.6056 | 1.6100 | 0.0311 | `results/powermlp_cifar10_default_20260919_041145` |

These single 3-epoch runs pointed the same way as the 10-epoch, 3-seed grid above; they
are not the evidence for the default.
...
1. **`--architecture deep` and `--k 3` can diverge without batch normalization.** With
   ...
   on the first 1024 real training samples, seeds 0, 1, 2, untrained models. **Measured
...
2. **The batch-normalization guard is measured in training mode.** At init the moving
   ...
   128-sample training batches, so BN ratios (1.05 to 1.37 measured) sit a little above the
...
7. **Per-dataset batch-normalization default.** MNIST on, CIFAR-10 off, each from its own
   grid (above). The MNIST evidence is thin (+0.00243 against a 0.002 bar, 3 seeds). Override
   with `--batch-normalization` / `--no-batch-normalization`; the resolved value is in

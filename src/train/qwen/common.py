"""Shared building blocks for the Qwen3 Pattern-3 (subword CLM) trainer.

**Scope (D-001, hard boundary): `Qwen3` plain causal LM ONLY.** The
``qwen/`` package hosts three distinct model surfaces in one directory --
``Qwen3`` (plain causal LM, this module's target), ``Qwen3Next``
(hybrid linear-attention MoE variant with its own routing/loss shape, in
``qwen3_next.py``), and ``Qwen3EmbeddingModel``/``Qwen3RerankerModel``
(contrastive retrieval, in ``qwen3_embeddings.py``). This module imports
ONLY from ``qwen3.py`` and must never import from ``qwen3_next.py`` or
``qwen3_embeddings.py`` -- see
``plans/plan-2026-09-12T173329-e20362c4/decisions.md`` D-001 for the full
reasoning (each of the other two surfaces needs a loss/data shape this plan's
findings never investigated; forcing either into this trainer's zamba2-clone
shape would silently ship a wrong-shaped trainer rather than a documented,
scoped-out gap).
...
Like ``Gemma3`` (and unlike ``Mamba2``, D-004: a pure encoder with no CLM
head, requiring a local functional wrapper), ``Qwen3.call()`` ALREADY bakes
its own independent, untied ``Dense`` LM head and returns a PLAIN TENSOR of
logits, shape ``(batch, seq_len, vocab_size)`` with the default
``return_dict=False`` (confirmed by direct read, ``qwen3.py:405-458``; also
...
* **No ``ClmPretrainConfig``/``load_train_val_datasets`` reuse.** That
  ...
  return ``{"logits": ...}`` (a dict). ``Qwen3.call`` returns a PLAIN TENSOR
  of logits, not a dict, so this module calls the leaf helpers directly
  (``load_wikipedia_train_val``, ``preprocess_clm_packed_dataset``,
  ``estimate_clm_steps_per_epoch``), the same as ``zamba2``/``mamba``/
  ``gemma``'s ``common.py``.

MoE knobs (``moe_layers``/``num_experts``/``num_experts_per_tok``/
``moe_intermediate_size``) are deliberately NOT exposed as CLI overrides
here: every shipped variant's ``moe_layers`` already targets valid indices
for that variant's own ``num_layers``, and the smoke-run knobs this trainer
does expose (``--num-layers``) would invalidate a variant's ``moe_layers``
list if it were left in place -- see :attr:`Qwen3TrainingConfig.variant_overrides`
for how a ``--num-layers`` override resets ``moe_layers`` to the model's own
...
VARIANT_NAMES: Tuple[str, ...] = tuple(Qwen3.MODEL_VARIANTS)
"""Every ``--variant`` choice: the four :data:`Qwen3.MODEL_VARIANTS` keys
(``30b-coder``, ``medium``, ``small``, ``tiny``), in declaration order."""
...
class Qwen3TrainingConfig:
    """Knobs for one Qwen3 pretraining run.
    ...
    :param max_seq_length: Tokens the MODEL sees per window. The packer is
        ...
        token to spend. Also forwarded as ``Qwen3``'s ``max_seq_len`` (the
        RoPE table horizon).
    ...
    :param hidden_size: Overrides the variant's hidden width. ``None`` keeps
        ...
        (``Qwen3.__init__`` raises ``ValueError`` otherwise) -- this config
    ...
    :param num_layers: Overrides the variant's block count. When set, the
        variant's ``moe_layers`` list (whose indices must be
        ``< num_layers``) is ALSO cleared to ``None`` in
        :attr:`variant_overrides`, so ``Qwen3.__init__`` falls back to its
        own default (``[]``, i.e. fully dense) instead of raising
        ``ValueError`` on an index that is now out of range against the
        ORIGINAL variant's ``moe_layers``.
    ...
    def variant_overrides(self) -> Dict[str, Any]:
        """:returns: The non-``None`` architecture overrides
            ...
            :meth:`Qwen3.from_variant`. When ``num_layers`` is overridden,
            ``moe_layers`` is ALSO set to ``None`` here so
            ``Qwen3.from_variant``'s ``config.update(kwargs)`` replaces the
            variant's original (now possibly out-of-range) ``moe_layers``
            list rather than leaving it in place -- ``Qwen3.__init__`` then
            falls back to its own default, ``[]`` (fully dense), instead of
            raising ``ValueError`` on an index `>= num_layers` (see the
            ``num_layers`` docstring above).
        :rtype: Dict[str, Any]
        """
...
def add_common_arguments(parser: argparse.ArgumentParser) -> argparse.ArgumentParser:
    """Register the shared Qwen3 training flags.
    ...
    parser.add_argument(
        ...
        help="Qwen3 model size to train.",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help="Override the variant's block count (also resets moe_layers "
             "to the model's dense default -- see Qwen3TrainingConfig).",
    )
    parser.add_argument(
...
def config_from_args(args: argparse.Namespace) -> Qwen3TrainingConfig:
    """Build a config from a parsed namespace.

    The ONE wiring site between :func:`add_common_arguments` and
    :class:`Qwen3TrainingConfig`. A flag that does not arrive here silently
    does nothing.
...
def build_datasets(
    ...
    ``train.mamba.common.build_datasets`` / ``train.gemma.common.build_datasets``.
    ...
    logger.info(
        f"Qwen3 corpus: {n_train} train / {n_val} val articles; "
...
def train(config: Qwen3TrainingConfig) -> Tuple[Qwen3, Any, str]:
    ...
    # zamba2/common.py's / mamba/common.py's / gemma/common.py's `train`.

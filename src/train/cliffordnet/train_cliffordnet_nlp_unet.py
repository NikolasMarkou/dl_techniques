...
The model (CliffordNetLMUNet) wraps:
...
from dl_techniques.models.cliffordnet.lmunet import CliffordNetLMUNet
...
class TrainingConfig:
    ...
    # Model
    variant: str = "nano"
    # Legacy single-stack knobs (kept for argparse compat with the non-UNet
    # script). Used only when ``variant == "custom"`` and the U-Net knobs
    # below are left at their defaults; otherwise they are ignored.
    channels: int = 128
    depth: int = 12
    ...
    # U-Net knobs (used when variant == "custom"; ignored otherwise).
    base_channels: int = 64
    stride_per_stage: List[int] = field(default_factory=lambda: [2, 2])
...
class StepCheckpointCallback(keras.callbacks.Callback):
    ...
    def __init__(
        ...
        analyze_every_steps: int = 50000,
        max_checkpoints: int = 3,
        model_name: str = "cliffordnet_nlp_unet",
        initial_step: int = 0,
...
def load_model_from_checkpoint(
    path: str,
) -> Tuple[CliffordNetLMUNet, int]:
    """Load a CliffordNetLMUNet from a ``.keras`` checkpoint.
...
def create_model(config: TrainingConfig) -> CliffordNetLMUNet:
    ...
    if config.variant in CliffordNetLMUNet.MODEL_VARIANTS:
        ...
        model = CliffordNetLMUNet(
            ...
            stride_per_stage=config.stride_per_stage,
...
def train_cliffordnet_nlp_unet(
    config: TrainingConfig,
) -> Tuple[CliffordNetLMUNet, keras.callbacks.History]:
    ...
    callbacks, results_dir = create_nlp_callbacks(
        model_name=f"CliffordNetLMUNet-{variant_label}",
        results_dir_prefix="cliffordnet_nlp_unet",
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument(
        ...
        choices=list(CliffordNetLMUNet.MODEL_VARIANTS.keys()) + ["custom"],
    ...
    # U-Net knobs (custom variant only).
    p.add_argument("--base-channels", type=int, default=64,
                    help="Top-of-U channel count (custom variant)")
    p.add_argument("--stride-per-stage", type=str, default="2,2",
                    help="Comma-separated strides between encoder levels (custom variant)")
    p.add_argument("--blocks-per-stage", type=str, default="2,2,2",
                    help="Comma-separated block counts per encoder/decoder level (custom variant)")
    p.add_argument("--bottleneck-blocks", type=int, default=2,
                    help="Number of bottleneck blocks (custom variant)")
    p.add_argument(
        "--tie-word-embeddings",
    ...
    # Output
    p.add_argument("--save-dir", type=str, default="results/cliffordnet_nlp_unet")
...
def _config_from_args(args: argparse.Namespace) -> TrainingConfig:
    ...
    return TrainingConfig(
        ...
        stride_per_stage=stride_per_stage,
...
def main() -> None:
    ...
    train_cliffordnet_nlp_unet(config)

"""CliffordNet bias-free denoiser training script.

Trains a bias-free CliffordNet denoiser following Miyasawa's theorem
requirements for optimal least-squares denoising. Uses file-based image
datasets with Gaussian noise augmentation and [-1, +1] normalization.
...
from dl_techniques.models.cliffordnet.denoiser import (
    CliffordNetDenoiser,
)
...
class TrainingConfig:
    """Configuration for CliffordNet denoiser training."""
    ...
    def __post_init__(self):
        if self.experiment_name is None:
            ...
            self.experiment_name = (
                f"cliffordnet_denoiser_{self.model_variant}_{timestamp}"
            )
...
def add_noise_to_patch(
    patch: tf.Tensor, config: TrainingConfig
...
class StreamingResultMonitor(keras.callbacks.Callback):
    ...
    def on_epoch_end(self, epoch: int, logs: Optional[Dict[str, Any]] = None):
        ...
        def _monitor_and_save(epoch_numpy):
            ...
            try:
                noisy_images, clean_images = add_noise_to_patch(
                    self.test_batch, self.config
                )
                denoised_images = self.model(noisy_images, training=False)

                if self.config.save_training_images:
                    self._save_image_samples(
                        epoch_val, noisy_images, clean_images, denoised_images
                    )

                mse_loss = tf.reduce_mean(tf.square(denoised_images - clean_images))
                psnr = tf.reduce_mean(
                    tf.image.psnr(denoised_images, clean_images, max_val=2.0)
                )
                logger.info(
                    f"Epoch {epoch_val} - Validation MSE: {mse_loss.numpy():.6f}, "
    ...
    def _save_image_samples(
        ...
        try:
            ...
            fig.suptitle(
                f"CliffordNet Denoiser - Epoch {epoch}", fontsize=20, y=0.98
            )
            ...
            for i in range(num_samples):
                ...
                denoised_img = np.clip(
                    (denoised[i].numpy() + 1.0) / 2.0, 0.0, 1.0
                )
...
def create_callbacks(
    ...
    common_callbacks, _ = create_common_callbacks(
        ...
        results_dir_prefix="cliffordnet_denoiser",
        monitor="val_loss",
...
def create_model_instance(
    ...
    """Create a CliffordNet denoiser model from config."""
    if config.model_variant in CliffordNetDenoiser.MODEL_VARIANTS:
        return CliffordNetDenoiser.from_variant(
            variant=config.model_variant,
            in_channels=input_shape[-1],
        )
    else:
        raise ValueError(
            f"Unknown model variant: {config.model_variant}. "
            f"Available: {list(CliffordNetDenoiser.MODEL_VARIANTS.keys())}"
        )
...
def train_cliffordnet_denoiser(config: TrainingConfig) -> keras.Model:
    """Train a bias-free CliffordNet denoiser and save clean inference model."""
    logger.info(f"Starting CliffordNet denoiser training: {config.experiment_name}")
    ...
    # MSE loss is critical for Miyasawa's theorem (least-squares optimal denoiser)
    model.compile(
        optimizer=optimizer,
        loss="mse",
        metrics=[
    ...
    try:
        inference_model = CliffordNetDenoiser.from_variant(
            variant=config.model_variant,
            in_channels=config.channels,
        )
        # Build with flexible spatial dims
        inference_model.build((None, None, None, config.channels))
...
def _verify_bias_free(model: keras.Model) -> None:
    ...
    if bias_layers:
        logger.warning(
            f"Bias-free check: {len(bias_layers)} layer(s) have bias/centering: "
            f"{bias_layers[:10]}"
        )
    else:
...
def parse_arguments() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Train CliffordNet Bias-Free Denoiser",
...
def main():
    ...
    logger.info(
        f"Config: model={config.model_variant}, epochs={config.epochs}, "
    ...
    try:
        model = train_cliffordnet_denoiser(config)

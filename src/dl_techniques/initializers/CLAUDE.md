...
- `orthonormal_initializer.py` — `OrthonormalInitializer`: orthonormal row matrices for signal norm preservation, plus a `gain` scale. Shape is `(n_clusters, feature_dims)` and is NOT transposed, so on a `Dense` kernel `(input_dim, units)` the constraint is `input_dim <= units` — only a WIDENING layer fits; it is a codebook/centroid initializer, and a non-2D shape raises `ValueError` (pinned as a contract by the two ConvNeXt block suites). Sign convention is `sign(diag(R))`, shared with `he_orthonormal_initializer.py` and `hypersphere_orthogonal_initializer.py`; the first-row convention it replaced folded row 0 into the positive orthant on every draw (mean cosine 0.801 to the all-ones direction). The QR is THIN (`O(d*k^2)`; the square one took 9.08 s and a 67.1 MB buffer at k=64, d=4096). **`validate_orthonormal_seed` / `validate_orthonormal_shape` live here and are imported by `he_orthonormal_initializer.py`** — they were verbatim copies in both
...
- `gabor_filters_initializer.py` — `GaborFiltersInitializer`: deterministic Gabor filter-bank initialization (Ozbulak & Ekenel) over a factorized orientation x scale x phase sweep (`sweep="diagonal"` recovers the paper's joint-`linspace` construction), DC-removed and energy-normalized to `sqrt(2/fan_in)` by default, with a `depthwise` flag selecting `fan_in = kh*kw*in_ch` (cross-channel, default `False`) vs `fan_in = kh*kw` (per-channel, `True` — the 4D kernel shape alone can't tell the two conventions apart). Two factories: `create_gabor_conv2d` (cross-channel, `trainable=True`, `depthwise=False` — the paper's warm start) and `create_gabor_depthwise_conv2d` (per-channel/depthwise, no cross-channel mixing, `trainable=False`, `depthwise=True`; output = `in_channels * filters_per_channel`; follow with a 1x1 Conv2D for a specific output count)
...
- `random_signs.py` — `RandomSigns`: every element drawn uniformly from `{-1, +1}` at ANY rank, via a stock `RandomUniform(-1, +1)` plus a `u >= 0` sign threshold. It goes through a stock initializer rather than `keras.random.*` because it runs inside `add_weight`, where there is no seed-generator variable for a direct `keras.random.*` call to update. It is the TabM paper's choice for the per-member ensemble scaling vectors (`ScaleEnsemble`, `LinearEfficientEnsemble`, `init_distribution='random-signs'`) and the only one of the three options that is BOTH symmetry-breaking and unit-magnitude: `'normal'` clusters members near the mean and `'ones'` makes every member's effective kernel identical at initialization. Not a general kernel init — the per-element variance is fixed at 1.0 with no fan-in scaling, so it belongs on a multiplicative scaling vector, not on a `Dense` kernel
- `clone.py` — `clone_initializer`: returns an INDEPENDENT initializer. A single *seedless* Keras 3 initializer INSTANCE self-assigns a seed and replays THE SAME UNDERLYING SAMPLE at every later site (measured: two `Dense(4)` sharing one instance are bit-identical; two built from the STRING `"glorot_uniform"` are not). Matching shapes are NOT the criterion -- a shorter draw comes out as the longer draw's prefix, so two weights of DIFFERENT shapes are the same random numbers (measured `r = 0.9999999999999964` at `(8,4,3,3,7)` then `(8,4,3,3)`; bit-identical with a non-scaling `RandomUniform`). Clone per site only where the two weights play DIFFERENT architectural roles. A *seeded* initializer still yields identical clones, deliberately

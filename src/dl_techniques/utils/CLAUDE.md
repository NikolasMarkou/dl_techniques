...
- `activation_serialization.py` — the canonical `activation`-argument (de)serialization pair, used directly by ~60 pre-existing files plus `layers/activations/common.py`'s wrapper trio (see `layers/CLAUDE.md`):
  ...
  - Consumed directly by ~60 pre-existing files and, as of this plan (`plan-2026-09-15T034909-a7edc8da`), migrated onto by 39 further files via `layers/activations/common.py`'s wrapper — see `layers/CLAUDE.md` for why those 39 go through the wrapper rather than this pair directly

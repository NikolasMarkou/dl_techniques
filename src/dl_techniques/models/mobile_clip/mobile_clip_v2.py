...
Pairs the faithful FastViT (MCi) image tower of
:mod:`dl_techniques.models.fastvit` with the OpenCLIP-shaped text transformer of
:mod:`.components` — the same one v1 uses — and adds the CLIP epilogue:
L2-normalized features, a learnable temperature, and the symmetric logits matrix.

This is the **faithful** MobileCLIP port. Its sibling :mod:`.mobile_clip_v1` is
deliberately non-faithful on the image side (it substitutes
``keras.applications`` backbones for the MCi tower under its own D-001) and is
neither deprecated nor changed by this module. See the package ``README.md``
§"v1 vs v2".

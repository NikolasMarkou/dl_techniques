...
Every class carries `@register_dl_technique(...)` (from `dl_techniques.utils.keras_registration`), a complete `get_config`, and a `compute_output_shape`, so the standard round trip works. The package string is the defining module's dotted path with `models/`'s `vision_language/` family directory stripped: `dl_techniques.models.mobile_clip.mobile_clip_v1>MobileClipModel`, `dl_techniques.models.mobile_clip.mobile_clip_v2>MobileClipV2Model`, and `dl_techniques.models.mobile_clip.components><ClassName>` for `ImageProjectionHead`, `MobileClipImageEncoder` and `MobileClipTextEncoder`. The FastViT blocks come from `dl_techniques/layers/fastvit/` and register under `dl_techniques.layers.fastvit.<module>`. Pre-2026-08-29 archives still load through the legacy `Custom>ClassName` alias the helper also binds (repo-root `MIGRATIONS.md`).

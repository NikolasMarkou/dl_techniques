...
class ConvNeXtV1(keras.Model):
    ...
    A pure convolutional architecture that reaches Vision Transformer accuracy
    at matched FLOPs without any attention. A patchified stem feeds
    ``len(depths)`` stages of :class:`ConvNextV1Block` -- depthwise ``KxK``
    ...
    **Block internals (ConvNextV1Block -- transform only, returns F(x)):**
    ...
    .. code-block:: text
        ...
        x ──► DWConv K×K (K=7) ──► LayerNorm
              └─ large receptive field, stands in for attention
                        │
                        ▼
    ...
    **Drop-path ramp (GLOBAL, not per stage):**
    ...
    .. code-block:: text
        ...
        rate[i] = drop_path_rate · i / (sum(depths) - 1)

        block i:  0 ────────────────────────────► sum(depths)-1
        rate:    0.0 ──────────────────────────► drop_path_rate
                 └ stage 0 ┘└ 1 ┘└── 2 ──┘└ 3 ┘
    ...
    **Variants:**
    ...
    Example:
        ...
        >>> model = ConvNeXtV1.from_variant("large",
        ...                                 pretrained="/path/to.keras")

    Note:
        No pretrained ConvNeXt V1 weights are distributed with
    ...
    def _build_stage(self, stage_idx: int) -> None:
        """Build and assign one stage of ConvNeXt blocks.
    ...
    def call(
        ...
        block: :class:`ConvNextV1Block` is transform-only and returns ``F(x)``.
    ...
    def load_pretrained_weights(
        ...
        not ``self.load_weights(..., by_name=True)``, which handles mismatches
        gracefully when the number of classes differs or when loading a
        backbone without its top classifier.
        ...
        :param skip_mismatch: Whether to skip layers with mismatched shapes.
            ...
            to the inverse of the transfer helper's ``strict``.
        ...
        Example:
            >>> model = ConvNeXtV1.from_variant("tiny", num_classes=10)
            >>> model.load_pretrained_weights("convnext_tiny_imagenet.keras",
            ...                               skip_mismatch=True)
        """
    ...
    def from_variant(
        ...
        """Create a ConvNeXt V1 model from a predefined variant.

        :param variant: One of ``"cifar10"``, ``"tiny"``, ``"small"``,
            ``"base"``, ``"large"``, ``"xlarge"``.
        :type variant: str
        ...
        :param pretrained: If a string, a path to a local weights file to load.
            If True, raises ``NotImplementedError`` -- no public ConvNeXt V1
        ...
        :return: ConvNeXtV1 model instance.
        :rtype: ConvNeXtV1
...
def create_convnext_v1(
    ...
    Thin wrapper around :meth:`ConvNeXtV1.from_variant` exposing the most
    common construction arguments at module level.

    :param variant: Model variant, one of ``"cifar10"``, ``"tiny"``,
    ...
    :param pretrained: If a string, a path to a local weights file. If True,
        raises ``NotImplementedError`` -- no public ConvNeXt V1 weights ship

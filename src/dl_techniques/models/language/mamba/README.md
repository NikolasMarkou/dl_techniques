...
each repo's own `config.json` (`d_model`, `n_layer`) — not from a paper table and not
from each other.
...
also accepts `1.4b` and `2.8b` as aliases for `1.3b` and `2.7b` — the shapes are
identical — but they are not Mamba-2 size names.

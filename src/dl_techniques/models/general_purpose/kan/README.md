...
**At the class default (`init_scheme='glorot_inspired'`)** this does not happen: `KAN` auto-injects the variance-controlled initializer pair from `dl_techniques.initializers.create_kan_initializers` (Rigas et al., arXiv:2509.03417) into every layer, independently for each of `kernel_initializer`/`base_scaler_initializer` -- a layer's own config setting only one of the two still gets the other filled in, rather than silently keeping `KANLinear`'s degenerate default for it -- breaking the `base_scaler` symmetry above. Gradients flow from construction, without requiring `update_kan_grids` first — but a SEPARATE, real instability survives grid adaptation alone: an untrained-but-grid-adapted `small`-variant model, freshly constructed, produced a single corner-point output of 38,494.8 at `init_scheme=None` for a target function bounded in `[-1, 2]`, against roughly 1 (measured 0.9-3.3 across several variants/seeds) at the class default, over the identical grid-adaptation call. `update_kan_grids` remains recommended either way — it adapts each layer's knot range to the data it actually sees, which is a different, additive concern from the symmetry-breaking `init_scheme` fixes.

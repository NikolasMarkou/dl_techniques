...
class _SoftOrthogonalBaseRegularizer(keras.regularizers.Regularizer):
    ...
    Provides shared initialization, L1/L2 hybrid regularization,
    matrix scaling, and serialization logic. Subclasses implement
    _compute_deviation to define the specific orthogonality target.

    Parameters
    ----------
    lambda_coefficient : float
        Weight for orthogonality Frobenius norm term
    ...
    def _compute_deviation(
            self,
            wt_w: Union[keras.KerasTensor, Any],
        ...
        rank : int
            Size of the square Gram matrix

        Returns
        -------
        tuple of (deviation_tensor, num_elements)
            deviation_tensor: matrix whose squared Frobenius norm will be penalized
            num_elements: number of elements to scale by (for matrix scaling mode)
        """
    ...
    def __call__(self, x: Union[keras.KerasTensor, Any], **kwargs) -> Union[keras.KerasTensor, Any]:
        ...
        **kwargs : Any
        ...
        if self._use_lambda:
            deviation, num_elements = self._compute_deviation(wt_w, eye, rank)
            frob_norm_sq = ops.sum(ops.square(deviation))

            if self._use_matrix_scaling:
                scaling_factor = ops.maximum(
                    ops.cast(num_elements, dtype=x.dtype),
                    ops.cast(EPSILON, dtype=x.dtype),
                )
                scaled_loss = ops.divide(
                    ops.multiply(self._lambda_coefficient, frob_norm_sq),
                    scaling_factor,
                )
                result = ops.add(result, scaled_loss)
            else:
                lambda_loss = ops.multiply(self._lambda_coefficient, frob_norm_sq)
...
class SoftOrthogonalConstraintRegularizer(_SoftOrthogonalBaseRegularizer):
    ...
    by minimizing the squared Frobenius norm of the off-diagonal elements of W^T * W.
    ...
    **kwargs : Any
    ...
    def _compute_deviation(self, wt_w, eye, rank):
        """Mask off-diagonal elements of the Gram matrix.

        Returns the off-diagonal portion of W^T*W and the count
        of off-diagonal elements for scaling.
        """
        off_diagonal_mask = ops.subtract(ops.cast(1.0, dtype=wt_w.dtype), eye)
        num_elements = ops.subtract(ops.multiply(rank, rank), rank)
        return ops.multiply(wt_w, off_diagonal_mask), num_elements
...
class SoftOrthonormalConstraintRegularizer(_SoftOrthogonalBaseRegularizer):
    ...
    by minimizing the squared Frobenius norm of (W^T * W - I).
    ...
    use_matrix_scaling : bool, optional
        Whether to scale regularization by matrix size for consistent effect
        across different sized layers, by default True
    **kwargs : Any
        Additional arguments passed to parent regularizer
    ...
    def _compute_deviation(self, wt_w, eye, rank):
        """Compute deviation from identity: W^T*W - I.

        Returns the full deviation matrix and the total element count
        for scaling.
        """
        num_elements = ops.multiply(rank, rank)
        return ops.subtract(wt_w, eye), num_elements

...
@register_dl_technique("dl_techniques.layers.pooling.downsample_and_skip")
class DownsampleAndSkip(keras.layers.Layer):
    """Produce ``(skip, downsampled)`` for one encoder junction.

    ``pool_type`` selects one of four weightless-or-weighted downsample branches,
    all inline rather than as a separate flag:

    - ``'max'`` (default) / ``'average'``: the skip is the pre-downsample tensor
      itself, and downsampling is ``MaxPooling2D(2, 2)`` / ``AveragePooling2D(2, 2)``.
    - ``'strided_conv'``: the skip is again the raw input; downsampling is a
      channel-preserving ``Conv2D(kernel_size=2, strides=2)`` -- the only branch
      that carries weights, with ``filters`` taken from the input channel count at
      build time, not fixed at construction.
    - ``'laplacian'``: a channel-preserving, bias-free
      ...
      half-resolution low band continues down the encoder.

    Average pooling and a bias-free strided conv are both linear, keeping the
    encoder path linear where a bias-free / Miyasawa denoiser arm needs that; the
    Laplacian split is linear too.

    Output order is ``(skip, downsampled)`` on every branch: the Laplacian branch
    ...
    :param pool_type: ``'max'`` (default), ``'average'``, ``'strided_conv'`` or
        ``'laplacian'``. ``'strided_conv'`` preserves the channel count rather
        than widening it, unlike the deleted ``ConvUNextModel._downsample``,
        since callers already widen channels as a separate step.
    :type pool_type: str
    :param laplacian_kernel_size: Gaussian blur kernel ``(height, width)`` handed to
        :class:`LaplacianPyramidLevel`. Inert unless ``pool_type='laplacian'``,
        but still recorded in ``get_config`` so a config round trip cannot
        silently lose it.
    ...
    :raises ValueError: If ``pool_type`` is not one of ``'max'``, ``'average'``,
        ``'strided_conv'`` or ``'laplacian'``.
    ...
    _VALID_POOL_TYPES = ("max", "average", "strided_conv", "laplacian")

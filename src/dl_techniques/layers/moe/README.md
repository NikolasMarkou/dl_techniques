...
- **Hard routing is sparse in FLOPs; wall-clock does not always follow.** Each expert runs only on the tokens routed to it (gather → FFN → scatter-add), an exact `num_experts / top_k` reduction in expert-token pairs (measured). But both kernels issue one FFN call per expert, and the sparse one adds a gather/scatter to each, so at small token counts the launch overhead can outweigh the saving. Measured on an RTX 4070 under `tf.function`, `d_model=512`, `hidden=1024`: 2048 tokens → `0.913x` at `num_experts=64` and `0.871x` at `128` (slightly **slower**); 8192 tokens → `2.108x` at `num_experts=64`; and at the `qwen3_next` preset's expert COUNT (`num_experts=512`, `top_k=10`, 2048 tokens, still at `d_model=512`/`hidden=1024`) sparse completes in 179 ms where the dense kernel exhausts the 12 GB device. Re-measured at the preset's real feature dims (`hidden_size=2048`, `moe_intermediate_size=512`), EAGER rather than under `tf.function`, on an idle RTX 4070: sparse **11.808 s** for a `(1, 2048, 2048)` output, dense `ResourceExhaustedError` — `OOM when allocating tensor with shape[2048,2048] ... [Op:BiasAdd]` at `InUse: 10638900480` of `Limit: 10654449664`. The two figures are different regimes, not a contradiction; the dense arm exhausts the device in both. Memory, unlike time, is reduced at every size.
- **The dense kernel is retained on purpose.** `MixtureOfExperts._process_hard_routing_dense` runs every expert on every token and masks the result. It is the numerical oracle the sparse kernel is gated against (`atol=1e-5`, `rtol=0`, `tests/test_layers/test_moe/test_the_sparse_kernel_matches_the_dense_oracle.py`), with the noise path live as well as off — the `add_noise=True` cells pin the noise DRAW rather than the seed, because Keras' `SeedGenerator` is not re-pinned by `set_random_seed` mid-process, and it is also the kernel that runs whenever `top_k >= num_experts`, where there is no sparsity to exploit.

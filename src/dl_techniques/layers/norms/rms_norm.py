...
class RMSNorm(keras.layers.Layer):
    ...
    - More computationally efficient (no mean computation for centering)
    - Better numerical stability in mixed precision training
    - Simpler gradient computation
    - Approximately 10-15% faster than LayerNorm in practice

    Args:
        axis: Integer or tuple of integers, axis or axes along which to compute RMS statistics.
        ...
        scale_initializer: String or keras.initializers.Initializer, initializer
    ...
    Example:
        Basic usage in transformer blocks:
        ...
        def transformer_block(inputs, hidden_size=768):
            ...
            norm_inputs = RMSNorm(epsilon=1e-5)(inputs)
        ...
        class LLaMABlock(keras.layers.Layer):
            def __init__(self, hidden_size=4096, **kwargs):
                ...
                self.attention = keras.layers.MultiHeadAttention(
                    num_heads=32,
                    key_dim=hidden_size // 32
                )
                self.feed_forward = keras.layers.Dense(hidden_size * 4, activation='silu')
    ...
    def build(self, input_shape: Tuple[Optional[int], ...]) -> None:
        ...
        It creates the scale parameter (if enabled) using add_weight().
        ...
        if self.use_scale:
            ...
            # CREATE the layer's scale parameter using add_weight()

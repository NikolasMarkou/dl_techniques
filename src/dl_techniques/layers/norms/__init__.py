...
the normalized axis away. Measured on a ``(3, 5, 8)`` input at the default
``axis=-1``: ``decoupled_max_logit`` returns a 3-tuple of ``(3, 5)`` tensors,
``dml_plus_center`` a 2-tuple shaped ``(3, 5)`` and ``(3, 5, 1)`` -- its norm
factor keeps the reduced axis at size 1 -- and ``dml_plus_focal`` a single

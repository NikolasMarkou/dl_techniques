...
| `window_band` | `WindowAttention` [^win] | 1-D **symmetric** sliding band over the token sequence: query `i` attends key `j` iff `abs(i - j) <= window_size`, where `window_size` is a HALF-WIDTH IN TOKENS. No grid folding, no square padding. Cost is `O(N^2)`, the same order as `multi_head` — it removes `window`'s `O(W^4)` floor, it is **not** `O(N*W)`. | Text encoders with local layers (ModernBERT / Longformer): pass `local_attention // 2`. | `(batch, seq_len, dim)` |

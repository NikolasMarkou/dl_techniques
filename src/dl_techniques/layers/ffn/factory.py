...
def validate_ffn_config(ffn_type: str, **kwargs: Any) -> None:
    ...
    if missing_params:
        raise ValueError(
            f"Missing required parameters for {ffn_type}: {missing_params}. "
    ...
    activation_params = ['activation', 'branch_activation', 'combination_activation']
...
def create_ffn_layer(
    ...
    # Add optional parameters with defaults if not provided
    for param_name, default_value in ffn_info['optional_params'].items():
        params[param_name] = kwargs.get(param_name, default_value)

    # Add all provided parameters (required and custom)

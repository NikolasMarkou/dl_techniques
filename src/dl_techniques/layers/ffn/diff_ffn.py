"""
A dual-pathway feed-forward network, ``DifferentialFFN``, that subtracts two
branches.

The input splits into a positive part and a negative part
(``x_pos = ReLU(x)``, ``x_neg = ReLU(-x)``), each runs through its own
Dense-LayerNorm-activation-Dense-activation branch with independent
weights, and the two branch outputs are subtracted before a final
projection: ``y = g(f_pos(max(0, x)) - f_neg(max(0, -x)))``. The model has
to express its function as a difference of two non-negative component
functions, following push-pull processing in biological sensory systems.
The kernels are unregularized unless you pass a ``kernel_regularizer``; a
``SoftOrthonormalConstraintRegularizer`` keeps each branch well conditioned.

References:
    - Shang et al., 2016. Understanding and Improving Convolutional
...
class DifferentialFFN(keras.layers.Layer):
    ...
    def __init__(
        ...
        **kwargs: Any
        ...
        # branch, never the shared instance -- a shared instance made the two branches start bit-identical. See decisions.md.
    ...
    def build(self, input_shape: Tuple[Optional[int], ...]) -> None:
        ...
        # The negative branch shares the positive branch's output shape.

...
def get_embedding_info() -> Dict[str, Dict[str, Any]]:
    ...
    ``EMBEDDING_REGISTRY``. The one object still shared with the registry is
    each entry's ``class`` value, which is the layer type itself, not mutable
    payload -- ``copy.deepcopy`` returns a class unchanged.

    :return: Mapping from embedding type key to a private copy of that
        type's registry entry.
    :rtype: Dict[str, Dict[str, Any]]
    """
    return copy.deepcopy(EMBEDDING_REGISTRY)

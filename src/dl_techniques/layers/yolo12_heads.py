...
class AnchorGenerator(keras.layers.Layer):
    ...
    on the input image dimensions and creates corresponding stride tensors.
    ...
    Args:
        ...
        strides_config: List of integers specifying the stride values for different
            feature map levels. Defaults to [8, 16, 32].
        **kwargs: Additional keyword arguments passed to the Layer base class.

    Input shape:
        Any tensor shape `(batch_size, ...)` - the input is used only to determine
        batch size for output tiling.

    Output shape:
        Tuple of two tensors:
        - anchors: `(batch_size, total_anchor_points, 2)` containing (x, y) coordinates
        - strides: `(batch_size, total_anchor_points, 1)` containing stride values

    Returns:
        Tuple[Tensor, Tensor]: A tuple containing:
            - anchors: Tensor with anchor point coordinates
    ...
    Example:
        ...
        >>> print(anchors.shape, strides.shape)
        (2, 8400, 2) (2, 8400, 1)
    """
    ...
    def _make_anchors(self) -> Tuple[ops.Tensor, ops.Tensor]:
        ...
        represents the center of a grid cell in the feature map.

        Returns:
            Tuple[ops.Tensor, ops.Tensor]: A tuple containing:
        ...
        anchor_points: List[ops.Tensor] = []
        stride_tensors: List[ops.Tensor] = []
        ...
        for stride in self.strides_config:
            ...
            x_coords = ops.arange(feat_w, dtype="float32") + 0.5
            y_coords = ops.arange(feat_h, dtype="float32") + 0.5
            ...
            stride_tensors.append(
                ops.full((feat_h * feat_w, 1), stride, dtype="float32")
            )
    ...
    def call(self, inputs, training: bool = None) -> Tuple[ops.Tensor, ops.Tensor]:
        """Forward pass returning tiled anchors and strides.

        The input is used only to determine the batch size for proper tiling
        of the anchor and stride tensors.

        Args:
            inputs: Input tensor (any shape with batch dimension).
            training: Boolean indicating training mode (unused but included
                for consistency).

        Returns:
            Tuple[ops.Tensor, ops.Tensor]: Tuple containing:
                - anchors: Tiled anchor coordinates `(batch_size, num_anchors, 2)`
                - strides: Tiled stride values `(batch_size, num_anchors, 1)`
        """
    ...
    def compute_output_shape(self, input_shape) -> Tuple[Tuple[int, ...], Tuple[int, ...]]:
        """Compute output shapes for anchors and strides.

        Args:
            input_shape: Shape of the input tensor.

        Returns:
            Tuple containing shapes of anchors and strides tensors.
        """

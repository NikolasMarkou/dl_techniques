...
-   **`ffn_type`** (default `None`) and **`ffn_args`**: `None` selects the built-in gated projection; anything else is built by `create_ffn_from_config`. `ffn_args` is applied last, so it wins on any key it shares with this layer's generic defaults. **A misspelled or unrecognized `ffn_args` key is silently dropped, not reported** — `create_ffn_layer` intersects the final parameter dict with the target FFN's signature and cannot tell a caller's explicit key from one of the layer's defaults, so the FFN is built with its default for that field. This is asymmetric with `q_norm_args`/`k_norm_args`/`v_norm_args`, which *do* raise on an unknown key because `create_normalization_layer` validates rather than filters.

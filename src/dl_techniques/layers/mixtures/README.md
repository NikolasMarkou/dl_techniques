...
- `'basis'` (the default) returns the raw kernel activations `phi_k(x) = exp(-gamma_k ||x - c_k||^2)`. **Unnormalized.** The exponent is clipped at `50.0` so `exp` cannot underflow, giving a floor of `exp(-50) ~ 1.9e-22` rather than `0.0`. This is the unchanged, historical behavior and remains the default.
- `'normalized'` returns the NRBF form `phi_k / sum_j phi_j`. Implemented as a softmax over the **unclipped** pre-exponential quantity (`softmax(-gamma_k ||x - c_k||^2)`), which is definitionally identical to the division but has no underflow failure mode — the naive `phi / sum(phi)` produces real `NaN` under `mixed_float16` for ordinary inputs.

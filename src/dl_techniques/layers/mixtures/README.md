...
- `'basis'` (the default) returns the raw kernel activations `phi_k(x) = exp(-gamma_k ||x - c_k||^2)`. **Unnormalized.** The exponent is clipped at `50.0`, which is retained but, at stock defaults, no longer engages.

  > **`'basis'` trains at all, but it does not train well — and only up to `D ~ 400`.** `gamma_init` defaults to `None` and is resolved **per mode** in `build()`, once the input's last dimension `D` is known: `1/D` under `'basis'`, `1.0` under `'normalized'`. An explicit float always wins over both.
  >
  > *Why the two arms differ.* `||x - c||^2` grows approximately linearly with `D` for standardized input (measured `E[||x-c||^2] / D` = 1.00 at `D` = 4 / 128 / 1024), so a fixed gamma that is sensible at `D = 4` drives the exponent `dist_sq * gamma` past the `50.0` clip at `D >= 64` and saturates **every** unit into the constant `exp(-50) = 1.93e-22` with gradient exactly `0.0` on both trainable weights. `'normalized'` is a shift-invariant softmax over `-gamma * dist_sq`, so only the between-unit logit *gaps* carry signal; those gaps are set by the center spread rather than by `D`, and a `1/D` gamma would shrink them ~`D`-fold and collapse the output toward a uniform `1/units`. Hence `'normalized'` keeps the historical `1.0`.
  >
  > *What the fix buys.* Measured, `units=8`, `normal(0,1)` input: gradmax on `(centers, gamma_raw)` goes from EXACTLY `0.0 / 0.0` to `3.6e-04 / 3.3e-02` at `D = 128` and `1.5e-04 / 3.4e-02` at `D = 256`; forward output std goes from `0.0` to `2.7e-02 .. 4.0e-02`. Pinned by `test_basis_mode_gradients_are_live_at_realistic_dimension` (D ∈ {4, 128, 256}).
  >
  > *Trains at all, NOT well.* At identical hyperparameters (`units=8`, `lr=1e-2`, 40 epochs, `D = 128`, a linearly separable binary fit) `'normalized'` reaches loss **0.176** while fixed `'basis'` sits at **0.6899** — chance is `0.693`. An epochs-only sweep (3 seeds, `units` and `lr` held fixed) clears neither the `0.40` target nor the `0.50` bar at any budget up to 400 epochs (worst seed `0.5273` @400). The binding constraint is the raw-`exp` parameterization, **not** the constant: `'basis'` sits at `0.691–0.694` for *every* `gamma_init` in `[1/128, 1.0]` at `D = 128`, while `'normalized'` moves `0.692 → 0.139` over the same sweep. Pinned by `test_basis_mode_fit_is_slow_at_realistic_dimension` (`xfail(strict=True)`).
  >
  > *Ceiling in `D` — read this before using `'basis'` on flattened images.* Because the resolved gamma is `1/D`, the `centers` gradient necessarily shrinks with `D` and eventually falls below this suite's own `MIN_USEFUL_GRADMAX = 1e-4` liveness floor. Measured, stock defaults, `units=8`, standardized input: `3.56e-04` (D=128), `1.50e-04` (D=256), `1.14e-04` (D=384), **`8.52e-05` (D=512)**, **`5.68e-05` (D=784)**, **`3.78e-05` (D=1024)**. So `'basis'` is usefully trainable only up to roughly **`D ~ 400`**, and **`D = 784` — flattened MNIST, the canonical toy for an RBF layer — is already past the ceiling.** Pinned by `test_basis_mode_gradient_falls_below_floor_at_high_dimension` (`xfail(strict=True)`). Do **not** try to buy headroom with a larger constant `c/D`: it is strictly worse (at `D = 512`, `c=1` → `8.3e-05`, `c=4` → `9.6e-07`, `c=8` → `1.5e-09`). A different parameterization, not a different constant, is what this needs.
  >
  > *Standardized input is a precondition.* The `1/D` law assumes `E[||x - c||^2] ~ D`, which holds for standardized data. The resolved exponent is then approximately `scale^2 + mean^2` — dimension-free, which is a genuine improvement over the old `D * gamma` trigger, but not a removal: it re-saturates the `50.0` clip once the input's scale or mean approaches `sqrt(50) ~ 7`. Measured at `D = 128` with stock defaults: `scale=2` → `3.14e-06` (already below the floor), `scale=4` → `1.46e-13`, `scale=10` → **exactly `0.0`** with forward std exactly `0.0`; `mean=7.1, scale=1` → **exactly `0.0`**. Being dimension-free, this hits `D = 4` too (`1.35e-06` at `scale=10`). Standardize your inputs. Pinned by `test_basis_mode_gradient_collapses_at_non_standard_input_scale`.
  >

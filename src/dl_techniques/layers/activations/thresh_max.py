...
def _compute_threshmax(
    ...
    Args:
        ...
        slope: Steepness of the gating function.
    ...
    gate = differentiable_step(confidence_diff, slope=slope, shift=0.0)
    ...
    return y_stepped / (total_sum + epsilon)
...
class ThreshMax(keras.layers.Layer):
    ...
    def __init__(
            ...
            slope_regularizer: Optional[Union[str, regularizers.Regularizer]] = L2_custom(-1e-3),
    ...
    def call(
        ...
        if self.trainable_slope and self.slope_weight is not None:
            effective_slope = self.slope_weight
        else:
            effective_slope = self.slope_initial_value

        return _compute_threshmax(inputs, self.axis, self.epsilon, effective_slope)

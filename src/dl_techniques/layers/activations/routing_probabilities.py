...
class RoutingProbabilitiesLayer(keras.layers.Layer):
    ...
    **Architecture**:
    ...
    def call(
        ...
        # Reshape to 2D: (batch_size, input_dim) by inferring the batch size.
        # This is more robust for JIT compilation (e.g., XLA).
        ...
        # Step 2: Initialize root probability using a JIT-friendly method
        # `ones_like` is preferred over `ones` with a dynamic shape for XLA.
        padded_probs = ops.ones_like(inputs_2d[:, 0:1])
        ...
        for i in range(self.num_decisions):
            ...
            # Reshape to flatten, creating interleaved structure. Use -1 for batch.

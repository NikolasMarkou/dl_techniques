...
- Docstrings carry mathematical formulations where relevant. **Two styles are in use — match the package you are editing, do not convert files wholesale:**
  - **Sphinx/reST (`:param:` / `:type:` / `:raises:`)** is the convention in `layers/` (245 of 283 modules), and is *mandatory* in `layers/attention/`, where all 34 modules use it and `channel_attention.py` is the reference exemplar (the count moved 33 -> 34 when `common.py` was added).

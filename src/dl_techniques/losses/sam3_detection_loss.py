"""Detection supervision for :class:`~dl_techniques.models.vision_language.sam.sam3.Sam3Image`.
...
``src/dl_techniques/models/vision_language/sam/sam3/*.py``, and every differentiable arithmetic
...
def box_cxcywh_to_xyxy(boxes: Any) -> Any:
    ...
    This conversion lives here because ``models/vision_language/sam/sam3/`` deliberately does not
...
class Sam3DetectionLoss(keras.losses.Loss):
    ...
    * ``- REFERENCE_ONLY(is_video_grounding / Q_det splits)``. Image-only path.
    ...
    :param num_aux_layers: (Keyword-position LAST in the signature, so no
        ...
        :func:`~dl_techniques.models.vision_language.sam.sam3.training_model.compile_sam3_trainer`

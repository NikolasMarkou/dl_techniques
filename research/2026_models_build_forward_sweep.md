...
| 17 | dino (v1/v2/v3) | ALL PASS | ALL PASS | smoke tests (FIXED plan_2026-06-15_39a31d4a + 2a23a001 + e2759fbc) | v1: `ClassTokenPrepend` ctor-order fix. v3: `.item()`->`float()` + smoke (plan_2a23a001). **v2: FIXED plan_2026-06-15_e2759fbc** — 13 bugs (B1-B7 + cascade #8-#13) made it forward; then a D-009 correctness/hardening extension: real learnable mask token (new `MaskTokenApply` layer, was a degenerate zero Dense-on-ones), removed the dead `is_training` input + fragile `call()` override → clean **2-input `[images, masks]`** contract. v2 forward `(2,10)` verified input+mask-sensitive; covers zeros/all/mixed-mask + register-enabled variant (registers position-free by design, Darcet 2023) + **`.keras` round-trip now PASSES** (the 2-input/MaskTokenApply changes made it serializable). 28 dino+embedding tests green. |
...
4. ~~**nano_vlm** — MultiModalFusion kwarg drift.~~ **RESOLVED** (kwargs, plan_2026-06-15_39a31d4a). Weight-tie CAVEAT **RESOLVED** in plan_2026-06-15_2a23a001: embedding/output tying re-implemented at CALL time (`matmul(x, transpose(word_embeddings.embeddings))`) at both logit sites (call + generate); smoke has a negative-proof tying assertion (zeroing `output_projection.kernel` leaves logits unchanged).
5. ~~**pft_sr** — PFTBlock `drop_path` kwarg.~~ **RESOLVED** (plan_2026-06-15_39a31d4a). PFTBlock tuple/list `build()` asymmetry **RESOLVED** in plan_2026-06-15_2a23a001: `build`/`compute_output_shape` now accept a tuple-of-shapes (robust check, not the naive widening which would break single-shape parsing).

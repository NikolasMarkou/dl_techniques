...
| 17 | dino (v1/v2/v3) | ALL PASS | ALL PASS | smoke tests (FIXED plan_2026-06-15_39a31d4a + 2a23a001 + e2759fbc) | v1: `ClassTokenPrepend` ctor-order fix. v3: `.item()`->`float()` + smoke (plan_2a23a001). **v2: FIXED plan_2026-06-15_e2759fbc** — 13 bugs (B1-B7: Dense-in-Lambda hoists, ClassTokenPrepend CLS, pos_embedding attr, flatten pos-embed Lambdas, ops.cond always-dict; + cascade #8-#13: batch-None ones, attention_type key, backbone name dedup, wrapper input-name collisions, masks input-is-output cycle, rank-0 is_training coercion). v2 3-input masked forward `(2,10)` PASS (zeros + all-mask). `.keras` round-trip (DINOHead) still separately broken. |

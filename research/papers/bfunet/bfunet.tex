...
\begin{tikzpicture}[
  ...
  \node[enc]  (e1)  at (0,-6.3)  {encoder L1\\[1pt] {\footnotesize $132$ ch, $3\times$ block}};
  % --- bottleneck (bottom center) ---
  \node[bott] (bn)  at (4,-8.2)  {bottleneck\\[1pt] {\footnotesize $264$ ch, $3\times$ block}};
  % --- decoder column (right, bottom to top) ---
  \node[dec]  (d1)  at (8,-6.3)  {decoder L1\\[1pt] {\footnotesize $132$ ch, $3\times$ block}};
  \node[dec]  (d0)  at (8,-4.2)  {decoder L0\\[1pt] {\footnotesize $66$ ch, $3\times$ block}};
  \node[io]   (out) at (8,0)     {output $\hat{x}=D(y)$\\[1pt] {\footnotesize $1{\times}1$, groups $=3$}};
  % --- skip nodes (high-frequency bands) ---
  \node[skip] (s0)  at (4,-4.2)  {hi-freq skip L0\\[1pt] $+\,3\times$ block};
  \node[skip] (s1)  at (4,-6.3)  {hi-freq skip L1\\[1pt] $+\,3\times$ block};
  % --- encoder flow ---
  \draw[arr] (in)  -- (gab);
  \draw[arr] (gab) -- (e0);
  ...
  % --- decoder flow ---
  \draw[arr] (bn)  -- node[lbl, below right=1pt] {upsample $\times2$} (d1);
  \draw[arr] (d1)  -- node[lbl, right] {upsample $\times2$} (d0);
  \draw[arr] (d0)  -- node[lbl, right] {linear head} (out);
  % --- high-frequency skip connections (horizontal) ---
  \draw[sarr] (e0) -- (s0);   \draw[sarr] (s0) -- node[lbl, above] {Add} (d0);
  \draw[sarr] (e1) -- (s1);   \draw[sarr] (s1) -- node[lbl, above] {Add} (d1);
\end{tikzpicture}%
}
\caption{\model{} data flow (channel widths shown). The encoder (left) descends; the decoder (right) ascends; the bottleneck sits at the base. A \textbf{frozen Gabor stem} feeds a two-level \convnext{}-V1 U-Net. Downsampling is a \textbf{Laplacian-pyramid split}: the low-frequency band descends (solid arrows) while the full-resolution high-frequency residual becomes the skip (dashed), passing through \textbf{three extra bias-free \convnext{} blocks} before a parameter-free channel-matched \texttt{Add} into the decoder. The head is a bias-free grouped $1{\times}1$ convolution (3 groups). Every operation is degree-1 homogeneous at inference, so $D(\alpha y)=\alpha D(y)$.}
\label{fig:arch}
\end{figure}

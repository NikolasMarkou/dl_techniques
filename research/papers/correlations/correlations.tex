...
\(D_{\mathrm{KL}}(p \,\|\, q)\) & Kullback--Leibler divergence of \(p\)
relative to \(q\) (\(= \mathbb{E}_p[\log p/q]\)). \\
...
\(\|\cdot\|_\mathcal{H}\) where specified). Single bars \(|\cdot|\) denote
...
distributions \(p, q\) with \(p \ll q\) (i.e., \(p\) absolutely continuous w.r.t.
\(q\); equivalently \(\mathrm{supp}(p) \subseteq \mathrm{supp}(q)\)): \[
D_{\mathrm{KL}}(p \,\|\, q) = \sum_x p(x) \log \frac{p(x)}{q(x)} \quad \text{(discrete)},
\qquad
D_{\mathrm{KL}}(p \,\|\, q) = \int p(x) \log \frac{p(x)}{q(x)}\,dx \quad \text{(continuous)}.
...
\] The decomposition \(I = H(X) + H(Y) - H(X,Y)\) is exact for discrete
variables. For continuous variables each differential-entropy term can be
infinite while \(I\) itself remains finite and well-defined --- the KL
form is the more fundamental quantity.
...
conditions and \emph{non-degeneracy} (\(\zeta_1 > 0\)),
\(\sqrt{n}(U_n - \theta) \to_d \mathcal{N}(0, k^2 \zeta_1)\)
where \(\zeta_1\) is the variance of the first conditional projection.
When the kernel is \emph{degenerate} (\(\zeta_1 = 0\)) the limit is instead
a weighted sum of \(\chi^2_1\) variables at rate \(n\) rather than \(\sqrt{n}\).
This degenerate regime is precisely what arises under the null \(H_0: X \perp Y\)
for dCor and HSIC --- so the actual engine for those null distributions (Part III)
is the \emph{degenerate} U-statistic limit, not the classical Hoeffding CLT above.
...
\item
  ...
  across rows, recompute the statistic. Under \(H_0: X \perp Y\) the joint
  sample is \emph{exchangeable}, so all permutations of the labels are
  equally likely and the empirical distribution of permuted statistics
  is the exact null distribution. Exchangeability --- not just marginal
  correctness --- is the load-bearing assumption; it fails for dependent
  observations (time series, repeated measures), where block permutation
  or stationary bootstrap is required instead. Used for \textbf{hypothesis tests}.
\item
...
\item
  ...
  any componentwise \emph{measurable bijection} of \(X\) and \(Y\)
  (diffeomorphisms in the continuous case); dCor under separate
  translation, orthogonal rotation, and global scaling per component
  (Sz\'ekely--Rizzo 2007); HSIC's invariance is entirely \emph{kernel-driven}
  --- translation-invariant kernels give translation invariance, rotation-invariant
  kernels (e.g. Gaussian RBF) give orthogonal invariance, and so on.
\item
...
10 & NMI & any × any (typ.\ cat × cat) & \([0, 1]\) & ✗ & ✓ & Bijective & \(O(n)\) &
\(O(rc)\) & ✗ (permutation) \\
11 & Transfer Entropy & time-series & \([0, \infty)\) & directed & ✗
(cond.\ on $Y$'s past) & Bijective & \(O(n \log n)\) KSG & \(O(n)\) & ✗
(surrogate) \\
...
\noindent\textbf{Caveats on the Range column.} Several entries describe
the \emph{population} or \emph{V-statistic} range. The bias-corrected
U-statistic dCor and the unbiased U-statistic HSIC can be \emph{negative}
in finite samples (their expectations are zero under independence, so
roughly half of null draws are negative). Chatterjee's \(\xi\) is in
\([0, 1]\) at the population level; the finite-sample range is
asymptotically \([-1/2 + o(1),\ 1]\). For Transfer Entropy and CMI the
listed \(O(n)\) / \(O(n \log n)\) time complexities assume KSG-style
neighbour search with a kd-tree; brute-force pairwise distance computation
costs \(O(n^2)\). PFI's "Sign \(\sim\)" reflects that the score-change is
itself signed but does not encode direction of effect on individual
predictions.
...
derivative \(z'(r) = 1/(1 - r^2)\) exactly cancels the factor, giving
\(\mathrm{Var}(z) \approx 1/n\); Fisher's \(1/(n-3)\) is the small-sample
correction (Fisher 1921).
...
\rho_S = \frac{6}{\pi} \arcsin\!\left(\frac{\rho}{2}\right) \approx 0.95\,\rho \text{ for moderate } |\rho|.
...
\] This form is borrowed from Pearson and is only asymptotic for
Spearman; under \(H_0\), \(\mathrm{Var}(\rho_S) = 1/(n-1)\) so the
simpler exact null statistic is \(\sqrt{n-1}\,\rho_S \to \mathcal{N}(0,1)\).
SciPy's \texttt{spearmanr} uses the \(t\)-approximation; the difference
matters only for small \(n\). Exact tables exist for small \(n\)
(Olds 1938); permutation tests are exact and the default for heavy ties.
...
\(\rho_S \approx 1 - 6(3.5)/(5 \cdot 24) = 1 - 21/120 = 0.825\). The
tie-corrected value (which SciPy reports) is slightly different
because of the midrank at \(y = 4\); for tied data, prefer
\texttt{spearmanr} over the shortcut formula.
...
and \(\tau\) is roughly \(\tfrac{2}{3} \rho_S\) in the \emph{small-\(\rho\)} regime
(the ratio \(\rho_S/\tau \to 3/2\) as \(\rho \to 0\)); for moderate to large \(\rho\)
the ratio drops toward \(\sim 1.25\) (e.g.\ \(\rho = 0.9\) gives \(\tau \approx 0.71\),
\(\rho_S \approx 0.89\)).

\subsubsection{3.5 Inference}\label{35-inference}

Under \(H_0\) of independence, \(\mathbb{E}[\tau] = 0\) and
\(\mathrm{Var}(\tau) = 2(2n + 5)/(9n(n-1))\); standardising,
\(\sqrt{(9n(n-1))/(2(2n+5))} \cdot \tau \to_d \mathcal{N}(0, 1)\). Exact
null distributions are tabulated for \(n \lesssim 50\) and used by
\texttt{scipy.stats.kendalltau(method="exact")} when \(n\) is small.
...
Using the population-SD convention (denominator \(n\), as in
\texttt{scipy.stats.pointbiserialr}), \(s_x = \sqrt{n_1 n_0}/n\) and
similarly for \(s_y\). The numerator
\(\sum_i (x_i - \bar{x})(y_i - \bar{y}) = (n_1 n_0 / n)(\bar{y}_1 - \bar{y}_0)\).
Substituting into \(r = \mathrm{Cov}(X,Y)/(s_x s_y)\) gives the formula
above. (Switching to the sample-SD convention with denominator \(n-1\)
replaces \(\sqrt{n_1 n_0/n^2}\) by \(\sqrt{n_1 n_0/(n(n-1))}\); the
identity to scipy is preserved by using the same convention for \(s_y\).)
...
\subsubsection{\texorpdfstring{4.5 The role of group balance, and a
common false bound}{4.5 The role of group balance, and a common false bound}}\label{45-bound-on-r_pb}

As a Pearson correlation, \(|r_{pb}| \leq 1\) universally
(Cauchy--Schwarz). It is sometimes claimed that
\(|r_{pb}| \leq \sqrt{n_1 n_0 / n^2}\); this is \emph{wrong}. The factor
\(\sqrt{n_1 n_0 / n^2}\) appearing in the formula is bounded by \(1/2\)
and tends to zero as groups become imbalanced, but it bounds only the
multiplier, not \(r_{pb}\) itself: the ANOVA identity
\(s_y^2 \geq (n_1 n_0 / n^2)(\bar{y}_1 - \bar{y}_0)^2\) precisely
cancels the factor and yields \(r_{pb}^2 \leq 1\). Zero within-group
variance gives \(r_{pb} = \pm 1\) at any split.

What \emph{does} happen with extreme imbalance is a loss of
\textbf{statistical power}: tiny groups make the standard error of
\(\bar{y}_1 - \bar{y}_0\) explode, so the same true effect produces
much smaller \(t\)-statistics and wider confidence intervals on
\(r_{pb}\). For this reason it is standard to also report a
sample-size-aware effect size such as Cohen\textquotesingle s \(d\)
alongside \(r_{pb}\).
...
\[
\phi_{\max} = \sqrt{\frac{p_{1+} q_{+1}}{q_{1+} p_{+1}}} \qquad
(\text{assuming } p_{1+} \leq p_{+1}; \text{ swap rows otherwise})
...
bias-corrected: \[
\tilde{V} = \sqrt{\max\!\left(0, \frac{\chi^2/n - (r-1)(c-1)/(n-1)}{\min(\tilde{r}-1, \tilde{c}-1)}\right)}, \quad \tilde{k} = k - \frac{(k-1)^2}{n-1} \text{ for } k \in \{r, c\}.
...
(Miller--Madow correction, 1955); note the sign --- the plug-in
estimator is biased \emph{upward}, so naive plug-in MI \emph{over}-estimates
dependence and Miller--Madow subtracts the estimated bias.
For small \(n\) and large alphabets the bias dominates; for \(n \gg K_{XY}\)
it is negligible. Improved estimators (Chao--Shen, Grassberger, NSB,
James--Stein) reduce bias further at the cost of complexity.
...
\item
  ...
  Because the estimator itself cannot exceed \(\log K\), InfoNCE is
  \emph{incapable} of detecting \(I > \log K\) --- a hard ceiling that
  saturates at modest values of \(I\). Detecting \(I = b\) nats
  faithfully therefore requires batch size at least \(e^b\), which is
  exponential in the quantity being estimated (McAllester \&
  Stratos 2020 establish this as a fundamental limit of any
  distribution-free high-confidence lower bound on MI).
\item
...
# Pearson ~ 0; MI (KSG, k=3) typically lands in ~1.0-1.4 nats here.
\end{lstlisting}
...
mine = MINE(alpha=0.6, c=15)  # alpha: B(n)=n^alpha cap; c: clump factor for grid search
...
\item
  ...
  \emph{expected} MI under random labellings (analogous to the Adjusted
  Rand Index) before normalising. Concretely,
  \[
  \mathrm{AMI}(U, V) = \frac{I(U;V) - \mathbb{E}[I(U;V)]}{\mathrm{avg}(H(U), H(V)) - \mathbb{E}[I(U;V)]},
  \]
  where the expectation is over a hypergeometric model preserving the
  cluster-size marginals. NMI rewards finer partitions (more clusters
  $\to$ higher entropy $\to$ easier to be informative); AMI corrects this.
  Vinh et al.\ (2010) is the canonical reference.
\item
...
\mathrm{TE}_{X \to Y} = \tfrac{1}{2} \log \frac{\mathrm{Var}(Y_{t+1} \mid Y_t^{(k)})}{\mathrm{Var}(Y_{t+1} \mid Y_t^{(k)}, X_t^{(l)})} = \tfrac{1}{2}\, \mathrm{GC}_{X \to Y},
\] where \(\mathrm{GC}_{X \to Y} := \log \frac{\mathrm{Var}(Y_{t+1} \mid Y_t^{(k)})}{\mathrm{Var}(Y_{t+1} \mid Y_t^{(k)}, X_t^{(l)})}\)
is the (population) Granger-causality statistic; the sample F-statistic
used to \emph{test} \(\mathrm{GC} = 0\) is a separate object. So TE is the
\emph{nonlinear, non-parametric generalisation} of Granger causality.
...
\item
  \textbf{Stationarity and ergodicity} are both required. Stationarity
  alone is not enough: TE estimators replace ensemble averages by
  time averages along a single trajectory, which is only valid under
  ergodicity. Non-ergodic stationary processes (e.g.\ regime-switching
  systems that never mix) produce TE estimates that depend on which
  trajectory was sampled. Non-stationary series should be detrended
  or differenced before estimation.
\item
...
\item
  ...
  on a collider (the "v-structure" \(X \to Z \leftarrow Y\); the
  resulting bias when sampling on \(Z\) is sometimes also called
  Berkson's paradox) introduces dependence between independent
  parents. Example: \(X, Y\) independent Bernoulli(1/2),
  \(Z = X \oplus Y\); then \(I(X; Y) = 0\) but \(I(X; Y \mid Z) > 0\).
\end{itemize}
...
\item
  ...
  case. With \(k\) the chosen neighbour order, Chebyshev distances in
  \((X, Y, Z)\) space, and \(n_{xz}(i), n_{yz}(i), n_z(i)\) the counts
  of points within the marginal slabs at point \(i\),
  \[
  \widehat{I}(X; Y \mid Z) = \psi(k) - \frac{1}{n}\sum_i \big[\psi(n_{xz}(i)+1) + \psi(n_{yz}(i)+1) - \psi(n_z(i)+1)\big].
  \]
\item
...
\emph{every} conditional independence in the distribution is entailed
by d-separation in the DAG --- i.e., no two paths' effects cancel by
coincidence (the Markov assumption supplies the converse direction).

\subsubsection{12.6 Code}\label{126-code}

\begin{lstlisting}[language=Python]
# Install: pip install git+https://github.com/gregversteeg/NPEET.git  (not on PyPI)
import numpy as np
...
\] where the \emph{U-centred} distance matrix \(\tilde{A}_{ij}\) is
\[
\tilde{A}_{ij} = a_{ij} - \frac{1}{n-2}\sum_{k} a_{ik} - \frac{1}{n-2}\sum_{k} a_{kj} + \frac{1}{(n-1)(n-2)}\sum_{k,l} a_{kl}
\]
(denominators \(n-2,\ n-2,\ (n-1)(n-2)\)), and similarly for
\(\tilde{B}_{ij}\). \(U_n\) has expectation zero under independence
and is the recommended statistic for hypothesis testing. Note that
because \(U_n\) is centred at zero, the bias-corrected sample dCov
can be \emph{negative} for finite samples even though the population
dCov is non-negative.
...
\widehat{\mathrm{HSIC}}_{\mathrm{V}} = \frac{1}{(n - 1)^2}\, \mathrm{tr}(K H L H) = \frac{1}{(n - 1)^2}\, \mathrm{tr}(\tilde{K} \tilde{L}),
\] where \(\tilde{K} = HKH\) is the centred kernel matrix. This is the
biased \(V\)-statistic estimator with bias \(O(1/n)\); the
\emph{unbiased \(U\)-statistic} version (Song, Smola, Gretton et al.\
2012) --- which can take negative values under independence ---
is preferred for hypothesis testing.
...
Under \(H_0\) of independence the U-statistic estimator satisfies
\[
n \cdot \widehat{\mathrm{HSIC}}_{\mathrm{U}} \to_d \sum_{j} \lambda_j (Z_j^2 - 1),
\]
a centred infinite sum of weighted \(\chi^2_1\); the biased V-statistic
above has the un-centred limit \(\sum_j \lambda_j Z_j^2\) (i.e.\
shifted by the trace of the integral operator). \textbf{Gamma
approximation}: \(n \cdot \widehat{\mathrm{HSIC}}_{\mathrm{V}} \sim \mathrm{Gamma}(\alpha, \beta)\)
with the moment match \(\alpha = (\mathbb{E}\widehat{\mathrm{HSIC}})^2 / \mathrm{Var}(\widehat{\mathrm{HSIC}})\),
\(\beta = \mathrm{Var}(\widehat{\mathrm{HSIC}}) / \mathbb{E}\widehat{\mathrm{HSIC}}\)
(Gretton, Fukumizu, Teo, Song, Schölkopf, Smola 2008). Fast and
...
\tightlist
\item
  Range: population \(\xi \in [0, 1]\); the asymptotic finite-sample
  range is \([-1/2 + o(1),\ 1]\) (Chatterjee 2020 / 2021). Negative
  values reflect sampling noise around zero under independence and are
  not interpreted as anti-correlation.
\item
...
\item
  ...
  \textbf{exact} SHAP for tree ensembles \emph{in path-dependent mode},
  with complexity \(O(TLD^2)\) where \(T\) is trees, \(L\) leaves per
  tree, \(D\) depth. The \emph{interventional} mode
  (\texttt{feature\_perturbation="interventional"} with a background
  dataset of size \(R\)) is a Monte Carlo estimate that converges to
  interventional Shapley values as \(R \to \infty\), at complexity
  \(O(TLDR)\); it is not "exact" in the same sense as path-dependent.
...
\item
  ...
  \end{itemize}
\item
  PFI of feature \(i\) is the change in performance. In the
  \emph{score} convention (higher = better, e.g.\ \(R^2\), accuracy ---
  the default in \texttt{sklearn.inspection.permutation\_importance}),
  \(\mathrm{PFI}_i = s_0 - s_i\); a positive value means the feature
  helped. In the \emph{loss} convention (lower = better, e.g.\ MSE),
  \(\mathrm{PFI}_i = s_i - s_0\). Important features are positive in
  both conventions; noise features can show small negative values from
  permutation variance and are not a bug.
\item
...
\item
  ...
  \] where \(\Omega\) is a sparsity penalty. Ribeiro et al.\ (2016)
  use \(\Omega\) to \emph{select} the top \(K\) features via a LASSO
  path (the so-called K-LASSO step), then fit the final surrogate as
  a weighted \emph{ridge} regression on those \(K\) features --- not
  as a LASSO. The \texttt{lime} package's default
  \texttt{feature\_selection="auto"} switches between LASSO-path and
  highest-weight selection depending on \(K\).
\item
...
\item
  ...
  choice. \texttt{LimeTabularExplainer} defaults to
  \(\sigma = 0.75 \sqrt{\,\text{num features}\,}\) and, more
  consequentially, to \texttt{discretize\_continuous=True} ---
  continuous features are quantile-binned before sampling, which
  materially changes the explanation. Both defaults should be set
...
X = np.column_stack([
    ...
    rng.integers(0, 10, n).astype(float),   # 10 distinct ordered values (sklearn treats as ordered numeric, ~9 split thresholds)
...
# scikit-learn does not expose Breiman-style OOB MDA directly; use
# held-out permutation importance (the closest practical proxy), or
# compute manually from model.estimators_ and their bootstrap indices.
from sklearn.model_selection import train_test_split

Xtr, Xte, ytr, yte = train_test_split(X, y, test_size=0.3, random_state=0)
model = RandomForestRegressor(n_estimators=500, random_state=0).fit(Xtr, ytr)

# Permute on a held-out set, NOT on the training data (see section 19.3).
result = permutation_importance(model, Xte, yte, n_repeats=20, random_state=0)
...
\item
  ...
  analogue of MDI; typical default. For XGBoost, the per-split gain
  is the second-order Taylor approximation of the loss improvement,
  \[
  \mathrm{Gain} = \tfrac{1}{2}\!\left[ \tfrac{G_L^2}{H_L+\lambda} + \tfrac{G_R^2}{H_R+\lambda} - \tfrac{(G_L+G_R)^2}{H_L+H_R+\lambda} \right] - \gamma,
  \]
  where \(G_*, H_*\) are sums of first- and second-order gradients in
  the partition; total gain is the sum of these over splits using the
  feature.
\item
...
\item
  ...
  \item
    ...
    / HSIC / MI (general).
  \item
    Continuous--ordinal → Spearman, Kendall; \emph{polyserial} (not
    polychoric) if you believe a latent normal --- polychoric requires
    \emph{both} variables to be discretised latent-normals.
  \item
    Continuous--binary → Point-biserial (equivalent to Pearson on the
    0/1 coding); rank-biserial / Mann--Whitney for non-linear monotone.
  \item
    Binary--binary → \(\phi\); tetrachoric if you believe latent normal.
  \item
    Categorical--categorical → Cramér\textquotesingle s \(V\); MI / NMI.
  \item
    Multivariate--multivariate → dCor, HSIC, KCCA; partial dCor for
    conditional analogues.
  \item
    Time series with direction → \emph{Granger causality} for linear
    stationary VAR fits; \emph{transfer entropy} as the nonlinear,
    non-parametric generalisation.
  \item
    Conditional dependence → Partial correlation (linear), CMI,
    conditional HSIC / HSCIC (Park \& Muandet 2020), partial dCor
    (Sz\'ekely \& Rizzo 2014), conditional Chatterjee.
  \end{itemize}
\item
...
\item
  ...
  observed value as the \(p\)-value. Exact under exchangeability of the
sample under \(H_0\) (which holds for i.i.d.\ pairs under \(X \perp Y\);
fails for time series and repeated measures), up to Monte
...
\item
  \textbf{Benjamini--Hochberg (BH)}: sort \(p\)-values \(p_{(1)} \leq \dots \leq p_{(m)}\);
  let \(K\) be the \emph{largest} \(k\) with \(p_{(k)} \leq k\alpha/m\),
  and reject all hypotheses with rank \(\leq K\). Controls the
  \textbf{false discovery rate (FDR)} --- the expected proportion of
  false positives among the rejections --- when the test statistics
  are independent or positively dependent. More powerful than
  Bonferroni; the standard default for screening.
\item
  \textbf{Benjamini--Yekutieli (BY)} (2001): replace the threshold by
  \(p_{(k)} \leq k\alpha / (m \cdot H_m)\) with
  \(H_m = \sum_{i=1}^{m} 1/i \approx \log m\). Controls FDR under
  \emph{arbitrary} dependence between test statistics --- the
  appropriate choice when feature-pair correlation tests are clearly
  not independent. More conservative than BH.
\end{itemize}
...
\item
  ...
  without TreeSHAP.
\item
  \textbf{PFI}: aggregate only; not axiomatic; the score-change is
  \emph{signed} (positive when the feature helps) but does not encode
  direction of effect on individual predictions; fast.
\end{itemize}
...
effect of \(Z\) before computing Pearson on the residuals; in closed
form,
\[
r_{XY \cdot Z} = \frac{r_{XY} - r_{XZ} r_{YZ}}{\sqrt{(1 - r_{XZ}^2)(1 - r_{YZ}^2)}}.
\]
CMI removes the \textbf{full statistical} effect of \(Z\). They agree
under joint normality and diverge under non-linearity. \textbf{Use
partial correlation for linear-Gaussian intuition; CMI for general
...
\item
  ...
  \texttt{model.predict()}?
\item
  \textbf{Path-dependent vs interventional TreeSHAP}. The two modes
  use different baselines and different value functions; switching
  between them changes both \(\mathbb{E}[f(\mathbf{X})]\) and the
  \(\phi_i\). Mixing one mode's baseline with another's
  \(\phi_i\) breaks additivity.
\item
...
for fname in feature_names:
    ...
    hs_stat, hs_p = Hsic().test(
        ...
        reps=999, workers=-1, random_state=0,
    )
...
# shap >= 0.43 prefers `explainer(Xte)` which returns an Explanation object;
# `shap_values` here returns a plain numpy array for clarity in this walkthrough.
...
# NOTE: coefficients live in the standardised-feature space (each X column
# was rescaled to unit variance). Magnitudes are comparable across features
# (all on the same scale) but should not be read as "effect per raw unit of X".
beta = lasso[-1].coef_

print(f"\n{'feature':12s}  {'LASSO β (std)':>14s}")
for fname, b in zip(aug_names, beta):
    print(f"{fname:12s}  {b:>+14.3f}")
print(f"Chosen alpha (lambda): {lasso[-1].alpha_:.4f}")
\end{lstlisting}
...
\subsection{Foundations of dependence}\label{foundations-of-dependence}

\begin{itemize}
\tightlist
\item
  \textbf{R\'enyi, A. (1959)}, \emph{On measures of dependence}. Acta
  Math. Hung.\ --- Seven axioms a "good" dependence measure should
  satisfy; the foundational reference no measure in this guide fully
  satisfies.
\item
  \textbf{Hoeffding, W. (1948)}, \emph{A non-parametric test of
  independence}. Ann. Math. Stat.\ --- The \(D\)-statistic; historical
  antecedent of dCor and HSIC.
\item
  \textbf{Embrechts, P., McNeil, A., Straumann, D. (2002)},
  \emph{Correlation and dependence in risk management: properties and
  pitfalls}. In \emph{Risk Management}. --- The standard modern
  reference on copula-based dependence, marginal-invariance, and what
  goes wrong with linear correlation under heavy tails.
\end{itemize}
...
\item
  \textbf{Chatterjee, S. (2021)}, \emph{A new coefficient of
...
\item
  \textbf{Lei, J., G'Sell, M., Rinaldo, A., Tibshirani, R. J.,
  Wasserman, L. (2018)}, \emph{Distribution-free predictive inference
  for regression}. JASA --- LOCO with conformal-based importance
  intervals.
\item
  \textbf{Covert, I., Lundberg, S., Lee, S.-I. (2020)}, \emph{Understanding
  global feature contributions with additive importance measures}.
  NeurIPS --- SAGE, the global Shapley-based feature-importance
  framework.
\item
  \textbf{Apley, D., Zhu, J. (2020)}, \emph{Visualizing the effects of
  predictor variables in black box supervised learning models}. JRSS-B
  --- ALE and M-plots; the conditional and conditional-corrected
  analogues of PDP.
\item
  \textbf{Friedman, J., Popescu, B. (2008)}, \emph{Predictive learning
  via rule ensembles}. Ann. Appl. Stat.\ --- Friedman's \(H\)-statistic
...
\subsection{Further modern non-parametric measures and tests}\label{further-modern-tests}

\begin{itemize}
\tightlist
\item
  \textbf{Heller, R., Heller, Y., Gorfine, M. (2013)}, \emph{A
  consistent multivariate test of association based on ranks of
  distances}. Biometrika --- The HHG independence test; a powerful
  rank-of-distance alternative to dCor.
\item
  \textbf{L\'opez-Paz, D., Hennig, P., Sch\"olkopf, B. (2013)},
  \emph{The randomized dependence coefficient}. NeurIPS --- RDC; a
  scalable non-linear coefficient based on random non-linear
  projections + canonical correlation.
\item
  \textbf{Bergsma, W., Dassios, A. (2014)}, \emph{A consistent test of
  independence based on a sign covariance related to Kendall's tau}.
  Bernoulli --- The \(\tau^\star\) statistic.
\item
  \textbf{Azadkia, M., Chatterjee, S. (2021)}, \emph{A simple measure
  of conditional dependence}. Ann. Stat.\ --- Conditional Chatterjee
  coefficient and the FOCI feature-selection algorithm.
\item
  \textbf{Zhang, K., Peters, J., Janzing, D., Sch\"olkopf, B. (2011)},
  \emph{Kernel-based conditional independence test}. UAI --- KCIT.
\item
  \textbf{Shah, R., Peters, J. (2020)}, \emph{The hardness of
  conditional independence testing and the generalised covariance
  measure}. Ann. Stat.\ --- GCM; a benchmark conditional-independence
  test with quantitative inference guarantees.
\end{itemize}
...
\item
  \textbf{Benjamini, Y., Yekutieli, D. (2001)}, \emph{The control of
  the false discovery rate in multiple testing under dependency}. Ann.
  Stat.\ --- BY procedure (FDR control under arbitrary dependence);
  the appropriate default when feature-pair tests are not independent.
\item

...
Three trees are meant to line up by directory name: **83 leaf model packages** under
...
- **The factory convention is now near-universal, but this entry is kept because
  ...
  **83 leaf** packages: **76 of 83** bind a `create_*` in their own
  `<pkg>/__init__.py` and **80 of 83** declare a curated `__all__`, so the old
...
|---|---|---|
| Python files under `src/` | 1040 | `find src -name '*.py' \| wc -l` |
| Python files under `tests/` | 1103 | `find tests -name '*.py' \| wc -l` |
...
| `.py` in `src/dl_techniques/models/` | 283 | `find src/dl_techniques/models -name '*.py' \| wc -l` |
...
| `.py` in layers + models | 583 | `find src/dl_techniques/layers src/dl_techniques/models -name '*.py' \| wc -l` |
...
| **LEAF** model packages under `src/dl_techniques/models/` (a directory carrying an `__init__.py` with no `__init__.py`-bearing child) | 83 | `find src/dl_techniques/models -mindepth 1 -type d ! -path '*__pycache__*' \| while read -r d; do [ -f "$d/__init__.py" ] && [ -z "$(find "$d" -mindepth 2 -maxdepth 2 -name __init__.py)" ] && echo "$d"; done \| wc -l` |
...
| Leaf model packages with a non-empty `__init__.py` | 80 | `find src/dl_techniques/models -mindepth 1 -type d ! -path '*__pycache__*' \| while read -r d; do [ -f "$d/__init__.py" ] && [ -z "$(find "$d" -mindepth 2 -maxdepth 2 -name __init__.py)" ] && echo "$d"; done \| while read -r p; do [ -s "$p/__init__.py" ] && echo "$p"; done \| wc -l` |
| Leaf model packages declaring `__all__` | 80 | `find src/dl_techniques/models -mindepth 1 -type d ! -path '*__pycache__*' \| while read -r d; do [ -f "$d/__init__.py" ] && [ -z "$(find "$d" -mindepth 2 -maxdepth 2 -name __init__.py)" ] && echo "$d"; done \| while read -r p; do grep -q '__all__' "$p/__init__.py" && echo "$p"; done \| wc -l` |
| Leaf model packages carrying a `README.md` | 83 | `find src/dl_techniques/models -mindepth 1 -type d ! -path '*__pycache__*' \| while read -r d; do [ -f "$d/__init__.py" ] && [ -z "$(find "$d" -mindepth 2 -maxdepth 2 -name __init__.py)" ] && echo "$d"; done \| while read -r p; do [ -f "$p/README.md" ] && echo "$p"; done \| wc -l` |
...
| Test dirs under `tests/test_models/` | 83 | `find tests/test_models -mindepth 1 -maxdepth 1 -type d ! -name __pycache__ \| wc -l` |
...
| Files using `@keras.saving.register_keras_serializable` | 481 | `grep -rl "@keras.saving.register_keras_serializable" src/dl_techniques --include=*.py \| wc -l` |
| Files defining `get_config` | 482 | `grep -rl "def get_config" src/dl_techniques --include=*.py \| wc -l` |
| Files using the central logger | 341 | `grep -rl "utils.logger" src/dl_techniques --include=*.py \| wc -l` |
...
| Library modules using Sphinx `:param` OUTSIDE `src/dl_techniques/layers/attention/` | 367 | `grep -rl ":param " src/dl_techniques --include=*.py \| grep -vc "src/dl_techniques/layers/attention"` |
...
| **Leaf** model packages BINDING a `create_` in their own package init (what a caller sees) | 76 | `find src/dl_techniques/models -mindepth 1 -type d ! -path '*__pycache__*' \| while read -r d; do [ -f "$d/__init__.py" ] && [ -z "$(find "$d" -mindepth 2 -maxdepth 2 -name __init__.py)" ] && echo "$d"; done \| while read -r p; do grep -qE '^(from\|import) .*create_\|^ +create_\|^def create_' "$p/__init__.py" && echo "$p"; done \| wc -l` |
| …the same thing counted by a bare mention-grep, which has overcounted at every measurement: by one until the docstring-only `convnext_patch_vae` init was deleted (2026-08-10), by three on 2026-08-24, and by one today — **73** against a binding count of 72, because `common/power_sampling/` merely MENTIONS `create_` in its init docstring while binding nothing. The gap between this row and the one above it is the whole point of keeping both | 77 | `find src/dl_techniques/models -mindepth 1 -type d ! -path '*__pycache__*' \| while read -r d; do [ -f "$d/__init__.py" ] && [ -z "$(find "$d" -mindepth 2 -maxdepth 2 -name __init__.py)" ] && echo "$d"; done \| while read -r p; do grep -q 'create_' "$p/__init__.py" && echo "$p"; done \| wc -l` |

...
- Performance validation for hybrid architecture
...
class TestQwen3NextModelBasic:
    ...
    def test_model_creation(self, tiny_config: Dict[str, Any]):
        ...
        # Check blocks are Qwen3NextBlock instances
        for i, block in enumerate(model.blocks):
            assert isinstance(block, Qwen3NextBlock), f"Block {i} is not a Qwen3NextBlock"

        # Check LM head
        assert model.lm_head is not None
        assert model.lm_head.units == tiny_config['vocab_size']

    def test_block_structure(self, mini_config_with_moe: Dict[str, Any]):
        """Test that each block has correct structure (3 DeltaNet + 1 Attention)."""
        model = Qwen3Next(**mini_config_with_moe)

        for i, block in enumerate(model.blocks):
            # Check block has correct components
            assert hasattr(block, 'delta_norms'), f"Block {i} missing delta_norms"
            assert hasattr(block, 'delta_layers'), f"Block {i} missing delta_layers"
            assert hasattr(block, 'delta_moe_layers'), f"Block {i} missing delta_moe_layers"
            assert hasattr(block, 'attention_norm'), f"Block {i} missing attention_norm"
            assert hasattr(block, 'attention_layer'), f"Block {i} missing attention_layer"
            assert hasattr(block, 'attention_moe'), f"Block {i} missing attention_moe"

            # Check correct number of delta layers (3)
            assert len(block.delta_norms) == 3, f"Block {i} should have 3 delta norms"
            ...
            assert isinstance(block.attention_layer, GatedAttention), f"Block {i} attention layer is not GatedAttention"
    ...
    def test_model_with_moe(self, mini_config_with_moe: Dict[str, Any]):
        """Test model with Mixture of Experts at block level."""
        model = Qwen3Next(**mini_config_with_moe)
        ...
        # Check each block has MoE layers
        for i, block in enumerate(model.blocks):
            # Check delta MoE layers
            for j, delta_moe in enumerate(block.delta_moe_layers):
                if delta_moe is not None:
                    from dl_techniques.layers.moe import MixtureOfExperts
                    assert isinstance(delta_moe, MixtureOfExperts), f"Block {i} delta MoE {j} is not MixtureOfExperts"

            # Check attention MoE layer
            if block.attention_moe is not None:
                from dl_techniques.layers.moe import MixtureOfExperts
                assert isinstance(block.attention_moe,
                                  MixtureOfExperts), f"Block {i} attention MoE is not MixtureOfExperts"
    ...
    def test_training_mode(self, tiny_config: Dict[str, Any], sample_input: keras.KerasTensor):
        ...
        model = Qwen3Next(**tiny_config_with_dropout)
...
class TestQwen3NextModelConfigurations:
    ...
    def test_model_variants(self):
        ...
        model_tiny = Qwen3Next.from_variant("tiny")
        assert model_tiny.vocab_size == 151936
        assert model_tiny.hidden_size == 512
        assert model_tiny.num_layers == 3  # 3 blocks = 12 effective layers
        assert model_tiny.num_experts == 4

        # Test small variant
        model_small = Qwen3Next.from_variant("small")
        assert model_small.vocab_size == 151936
        assert model_small.hidden_size == 1024
        assert model_small.num_layers == 6  # 6 blocks = 24 effective layers
        assert model_small.num_experts == 8

        # Test 80b variant (configuration only, too large to instantiate)
        try:
            model_80b = Qwen3Next.from_variant("80b", num_layers=1, hidden_size=64)  # Override to tiny size
            assert model_80b.vocab_size == 151936
        except Exception:
    ...
    def test_stochastic_depth_configuration(self):
        ...
        for block in model.blocks:
            assert hasattr(block, 'stochastic_depth_layers')
...
class TestQwen3NextModelSerialization:
    ...
    def test_get_config_completeness(self, serializable_config: Dict[str, Any]):
        ...
        model = Qwen3Next(**serializable_config)
    ...
    def test_serialization_cycle_standard(self, serializable_config: Dict[str, Any]):
        ...
        original_model = Qwen3Next(**config)
        sample_input = keras.ops.convert_to_tensor(
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            model_path = os.path.join(tmpdir, 'qwen3_next_standard_test.keras')
    ...
    def test_serialization_cycle_moe(self, serializable_config: Dict[str, Any]):
        ...
        original_model = Qwen3Next(**serializable_config)
        sample_input = keras.ops.convert_to_tensor(
            np.random.randint(0, 500, size=(2, 20)),
            dtype='int32'
        )
        ...
        # Save and load model
        with tempfile.TemporaryDirectory() as tmpdir:
            model_path = os.path.join(tmpdir, 'qwen3_next_moe_test.keras')
            original_model.save(model_path)
            loaded_model = keras.models.load_model(model_path)
    ...
    def test_config_reconstruction(self, serializable_config: Dict[str, Any]):
        ...
        reconstructed_model = Qwen3Next.from_config(config)
...
class TestQwen3NextModelFactories:
    ...
    def test_model_variant_configurations(self):
        ...
        variants = ["tiny", "small", "80b", "80b_a3b"]

        for variant in variants:
            assert variant in Qwen3Next.MODEL_VARIANTS
            config = Qwen3Next.MODEL_VARIANTS[variant]
...
class TestQwen3NextModelErrorHandling:
    ...
    def test_invalid_factory_parameters(self):
        ...
        with pytest.raises(ValueError, match="Unknown task_type"):
            create_qwen3_next("tiny", task_type="invalid_task")
...
class TestQwen3NextModelPerformance:
    ...
    def test_effective_layer_counting(self):
        """Test that effective layer count is correct (4 per block)."""
        config = {
            'vocab_size': 500,
            'hidden_size': 64,
        ...
        # Each block should have 3 delta + 1 attention = 4 effective layers
        total_effective_layers = 0
        for block in model.blocks:
            total_effective_layers += 3  # delta layers
            total_effective_layers += 1  # attention layer

        assert total_effective_layers == 12  # 3 blocks × 4 layers = 12
    ...
    def test_moe_parameter_scaling(self):
        ...
        # Model without MoE
        config_no_moe = {**base_config, 'num_experts': 1, 'num_experts_per_tok': 1}
        model_no_moe = Qwen3Next(**config_no_moe)
        ...
        model_with_moe = Qwen3Next(**config_with_moe)
    ...
    def test_forward_pass_efficiency(self):
        ...
        model = Qwen3Next.from_variant("tiny")
    ...
    def test_memory_usage_scaling(self):
        ...
        configs = [
            ...
            {'hidden_size': 96, 'num_layers': 2, 'num_attention_heads': 6},
        ]
...
class TestQwen3NextIntegration:
    ...
    def test_model_summary_information(self):
        ...
        # Each block should be properly configured
        for i, block in enumerate(model.blocks):
            assert hasattr(block, 'dim')
            assert hasattr(block, 'num_heads')
            assert block.dim == model.hidden_size

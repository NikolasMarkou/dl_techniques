...
class TestCLIPTextTowerCausality:
    ...
    class _Tap(keras.layers.Layer):
        """Pass-through that records the sequence it is handed."""
        ...
        def call(self, x, attention_mask=None, training=None):
            self._sink.append(keras.ops.convert_to_numpy(x))
            return x

    def _tapped_model(self):
        model = CLIP(
        ...
        model.encode_text(x, training=False)  # build before tapping

        sink = []
        model.text_transformer_layers.append(self._Tap(sink))

        x2 = x.copy()
        x2[:, self.PERTURB_AT] = (x2[:, self.PERTURB_AT] + 7) % 31 + 1
        model.encode_text(x, training=False)
        model.encode_text(x2, training=False)
        return np.asarray(sink[0]), np.asarray(sink[1])

    def test_future_token_does_not_change_the_past(self):
        a, b = self._tapped_model()

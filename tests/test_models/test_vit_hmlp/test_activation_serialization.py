...
must undo it -- ViTHMLP sibling of the ``vit_siglip`` defect (D-012, N-1).
...
- ``ViTHMLP`` HAD a ``from_config``, and it still missed this key -- it handled
  only initializers and regularizers. The presence of a ``from_config`` is not
  evidence that every serialized key is covered by it.
...
class TestViTHMLPActivationSerialization:

...
from dl_techniques.layers.convnext_v2_block import (
    ConvNextV2Block,
    ConvNextV2Config
)
...
@pytest.fixture
def block_config() -> ConvNextV2Config:
...
# ConvNextBlock tests
def test_block_initialization(block_config: ConvNextV2Config, block_params: Dict[str, Any]) -> None:
    """Test initialization of ConvNextBlock."""
    block = ConvNextV2Block(conv_config=block_config, **block_params)
...
def test_block_output_shape(sample_inputs: tf.Tensor, block_config: ConvNextV2Config,
...
def test_block_output_shape_with_stride(sample_inputs: tf.Tensor, block_config: ConvNextV2Config,
                                        block_params: Dict[str, Any]) -> None:
    """Test if block correctly changes output shape when using stride > 1."""
    # Create config with stride 2
    strided_config = ConvNextV2Config(
    ...
    block = ConvNextV2Block(conv_config=strided_config, **block_params)
    outputs = block(sample_inputs)
...
def test_block_training_behavior(sample_inputs: tf.Tensor, block_config: ConvNextV2Config,
                                 block_params: Dict[str, Any]) -> None:
...
def test_block_without_dropout(sample_inputs: tf.Tensor, block_config: ConvNextV2Config) -> None:
...
def test_block_serialization(block_config: ConvNextV2Config, block_params: Dict[str, Any]) -> None:
    """Test serialization of ConvNextV2Block with more robust property comparison."""
    # Create and build the original block
    original_block = ConvNextV2Block(conv_config=block_config, **block_params)
...
def test_block_activations(activation: str, block_config: ConvNextV2Config,
    ...
    block = ConvNextV2Block(conv_config=act_config, **block_params)
    output = block(sample_inputs)
    assert not tf.reduce_any(tf.math.is_nan(output))


def test_block_gradient_flow(sample_inputs: tf.Tensor, block_config: ConvNextV2Config,
                             block_params: Dict[str, Any]) -> None:
...
def test_gamma_scaling(sample_inputs: tf.Tensor, block_config: ConvNextV2Config, block_params: Dict[str, Any]) -> None:
...
def test_model_integration(sample_inputs: tf.Tensor, block_config: ConvNextV2Config,
                           block_params: Dict[str, Any]) -> None:
...
def test_model_save_load(sample_inputs: tf.Tensor, block_config: ConvNextV2Config,

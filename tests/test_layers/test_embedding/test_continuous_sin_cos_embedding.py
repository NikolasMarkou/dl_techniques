...
class TestContinuousSinCosEmbed:
    ...
    def test_get_config_round_trip(self):
        layer = ContinuousSinCosEmbed(dim=48, ndim=3, max_wavelength=5000.0)
    ...
    def test_assert_positive_is_not_serialized(self):
        assert "assert_positive" not in ContinuousSinCosEmbed(dim=48, ndim=3).get_config()
    ...
    def test_assert_positive_is_rejected_by_the_constructor(self):
        ...
        with pytest.raises(ValueError, match="assert_positive"):
            ContinuousSinCosEmbed(dim=48, ndim=3, assert_positive=False)

    def test_a_legacy_config_carrying_assert_positive_still_loads(self):
        stored = ContinuousSinCosEmbed(dim=48, ndim=3, max_wavelength=5000.0).get_config()
        legacy = dict(stored)
        legacy["assert_positive"] = False          # as an old archive has it
        rebuilt = ContinuousSinCosEmbed.from_config(legacy)
    ...
    def test_a_legacy_config_reloads_a_saved_model_unchanged(self, tmp_path):
        ...
        model = keras.Model(inp, ContinuousSinCosEmbed(dim=48, ndim=3)(inp))

...
class TestGLUFFN:
    """Comprehensive test suite for GLUFFN layer."""
    ...
    def test_initialization(self, layer_config: Dict[str, Any]) -> None:
        ...
        assert isinstance(layer.value_proj, keras.layers.Dense)
        ...
        assert layer.gate_proj.units == layer_config['hidden_dim']
        assert layer.value_proj.units == layer_config['hidden_dim']
    ...
    def test_serialization_cycle(self, layer_config: Dict[str, Any],
        ...
        outputs = GLUFFN(**layer_config)(inputs)
    ...
    def test_edge_cases(self) -> None:
        ...
        with pytest.raises(ValueError, match="hidden_dim must be a positive integer"):
            GLUFFN(hidden_dim=0, output_dim=32)

        with pytest.raises(ValueError, match="hidden_dim must be a positive integer"):
            GLUFFN(hidden_dim=-1, output_dim=32)

        with pytest.raises(ValueError, match="hidden_dim must be a positive integer"):
            GLUFFN(hidden_dim=1.5, output_dim=32)

        # Test invalid output_dim
        with pytest.raises(ValueError, match="output_dim must be a positive integer"):
            GLUFFN(hidden_dim=32, output_dim=0)

        with pytest.raises(ValueError, match="output_dim must be a positive integer"):
            GLUFFN(hidden_dim=32, output_dim=-1)

        # Test invalid dropout_rate
        with pytest.raises(ValueError, match="dropout_rate must be between 0 and 1"):
            GLUFFN(hidden_dim=32, output_dim=16, dropout_rate=-0.1)
    ...
    def test_different_activations(self, sample_input_3d: keras.KerasTensor) -> None:
        ...
        activations_to_test = ['swish', 'gelu', 'relu', 'tanh', 'sigmoid']
    ...
    def test_no_bias_configuration(self, sample_input_2d: keras.KerasTensor) -> None:
        ...
        assert layer.gate_proj.use_bias is False
        assert layer.value_proj.use_bias is False
    ...
    def test_multiple_instantiation(self, layer_config: Dict[str, Any],
        ...
        layer1 = GLUFFN(**layer_config, name='glu1')
        layer2 = GLUFFN(**layer_config, name='glu2')
    ...
    def test_gating_mechanism(self, sample_input_2d: keras.KerasTensor) -> None:
        """Test that the gating mechanism is working correctly."""
        # Create layer with sigmoid gate for easier interpretation
        layer = GLUFFN(
            hidden_dim=32,
            output_dim=16,
        ...
        gate = layer.gate_proj(sample_input_2d)
        value = layer.value_proj(sample_input_2d)

        # Apply activation to gate
        activated_gate = keras.activations.sigmoid(gate)
        ...
        np.testing.assert_allclose(
            ...
            rtol=1e-5, atol=1e-5,
            err_msg="Gating mechanism not working correctly"
        )
    ...
    def test_with_transformer_architecture(self, sample_input_3d: keras.KerasTensor) -> None:
        ...
        # Create a mini transformer block with GLU FFN
...
class TestGLUFFNIntegration:
    """Integration tests for GLUFFN in realistic scenarios."""

    def test_classification_model(self) -> None:
        """Test GLU FFN in a classification model."""
        # Create a simple classification model with GLU FFN
    ...
    def test_sequence_model(self) -> None:
        """Test GLU FFN in a sequence processing model."""
        seq_len, embed_dim = 50, 128

        inputs = keras.Input(shape=(seq_len, embed_dim))

        # LSTM followed by GLU FFN

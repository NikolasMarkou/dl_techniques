...
# ---------------------------------------------------------------------
#
# `reassociation_atol` and its constants now live in `tests/numerics.py` — a SECOND
# consumer appeared (`tests/test_layers/test_attention/test_energy_attention.py`), and a
# derivation copied into two files is the magic number this helper exists to prevent,
# one level up. The DECISION plan-2026-07-30T140922-8af1028f/D-024 anchor, the full
# derivation and the calibration table moved with it; read them there.
from tests.numerics import reassociation_atol  # noqa: E402


def _hopfield_reduction_lengths(hopfield: HopfieldNetwork):
    """`update()` is two chained contractions: `xi @ g` over `dim`, then `xi.T @ r` over K."""
    return (hopfield.dim, hopfield.hopfield_dim)


def _block_reduction_lengths(block: EnergyTransformer, num_tokens: int):
    """Contraction lengths of ONE `EnergyTransformer` descent step, read off the block.

    Derived from the two sub-layers' actual expressions, not guessed:

    - `EnergyAttention`: `w_query`/`w_key` project over ``embed_dim`` (2 x D); the logits
      contract over ``head_dim`` (Y); the softmax reduces over the key axis (N); `term_q`
      and `term_k` are each `einsum('yhd,byhn,bhnm->b.d')`, contracting (y, h, n) jointly.
    - `HopfieldNetwork`: D then K, per `_hopfield_reduction_lengths`.
    """
    d, y, h, k = block.embed_dim, block.head_dim, block.num_heads, block.hopfield_dim
    return (
        2 * d,                  # w_query / w_key projections
        y,                      # attention logits
        num_tokens,             # softmax over the key axis
        2 * y * h * num_tokens,  # term_q + term_k read-outs
        d + k,                  # the Hopfield update
    )
...
class TestHopfieldNetwork:
    ...
    def test_no_token_mixing(self, activation, sample_input):
        ...
        This catches token mixing that is not permutation-EQUIVARIANT: a positional or
        order-dependent leak between tokens (injecting `update += 0.1 * cumsum(update,
        axis=1)` moves the comparison to 3.96 for `relu` / 0.265 for `softmax`, four orders
        of magnitude above the derived bound below).

        **It does NOT catch a token-axis softmax, contrary to what this docstring claimed
        until 2026-07-30 — MEASURED, not reasoned.** Replacing `softmax(beta*h, axis=-1)`
        with `axis=1` (and `relu(h)` with `softmax(h, axis=1)`) in
        `HopfieldNetwork.update` leaves this test GREEN, with a difference of 3.58e-07 —
        pure roundoff (1.1 eps), below even the OLD `atol=1e-6`. Softmax over a set is
        permutation-equivariant, so `update(permute(g)) == permute(update(g))` still holds
        exactly. This is a pre-existing blind spot of the permutation probe, NOT a
        consequence of the derived tolerance below. The suite is not blind to that defect:
        the same injection fires `test_gradient_oracle` (all 4 parametrizations),
        `test_perturbing_one_token_leaves_the_others_alone` (both) and 6 mask/energy tests.
        Do not re-add the old claim here.
        """
        ...
        # `atol` is DERIVED, not pasted: permuting the token axis reassociates the two
        # contractions inside `update()`, so the two sides differ by float32 rounding even
        # when the layer is exactly right. See `reassociation_atol` (D-024) — the previous
        # `atol=1e-6` was below that floor and could never pass. `rtol=0` so the derived
        # bound is the ONLY tolerance in play.
        atol = reassociation_atol(
            _hopfield_reduction_lengths(hopfield),
            num_steps=1,
            scale=float(np.abs(u_plain).max()),
        )
        max_abs_diff = float(np.abs(u_perm - u_plain[:, perm, :]).max())
        np.testing.assert_allclose(
            u_perm, u_plain[:, perm, :], rtol=0.0, atol=atol,
            err_msg=(
                "HopfieldNetwork mixed tokens — update(permute(g)) != permute(update(g)) "
                f"by {max_abs_diff:.3e}, against a derived reassociation bound of "
                f"{atol:.3e}. Look for an order-dependent leak across the token axis (a "
                "shift, a cumulative sum, a positional weighting) — a real one is O(|update|)"
                ", four orders of magnitude above the bound, so this is NOT a tolerance "
                "problem. NOT a token-axis softmax: that is equivariant and invisible here "
...
class TestKerasMaskIsHonored:
    ...
    def test_pads_do_not_shift_real_token_outputs(self):
        model, block, emb_model = _padded_block_model()
        ...
        # `atol` is DERIVED, not pasted: `padded` and `short` have DIFFERENT sequence
        # lengths, so the backend blocks their reductions differently and the two outputs
        # differ by float32 rounding even under a perfectly honored mask. See
        # `reassociation_atol` (D-024) — the previous `atol=1e-6` was below that floor and
        # could never pass. `rtol=0` so the derived bound is the ONLY tolerance in play.
        atol = reassociation_atol(
            _block_reduction_lengths(block, int(padded.shape[1])),
            num_steps=block.num_steps,
            scale=float(np.abs(y_short).max()),
        )
        max_abs_diff = float(np.abs(y_pad - y_short).max())
        np.testing.assert_allclose(
            y_pad, y_short, rtol=0.0, atol=atol,
            err_msg=(
                ...
                "self.attention.energy/update (F-02b). A dropped mask lets the PAD tokens' "
                "real contribution through, which is O(|y|) — orders of magnitude above "
                "the bound, so this is NOT a tolerance problem."
            ),
        )
    ...
    def test_return_energy_with_a_propagated_mask(self):
        ...
        # Same derived bound as `test_pads_do_not_shift_real_token_outputs` — same block,
        # same shapes, same mechanism (D-024). `rtol=0`.
        atol = reassociation_atol(
            _block_reduction_lengths(block, int(padded.shape[1])),
            num_steps=block.num_steps,
            scale=float(np.abs(y_short).max()),
        ...
        np.testing.assert_allclose(
            ...
            err_msg=(
                ...
                f"return_energy=True path, against a derived reassociation bound of "
                f"{atol:.3e} — the mask is not reaching self.energy()/update()."
            ),
        )

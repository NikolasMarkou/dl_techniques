...
class TestWeightsBeforeFirstCall:
    def test_batch_norm_weights_are_exact_before_any_call(self):
        ...
        """
        layer = BasicBlock(filters=8, name="preweights")
        layer.build((B, 8, 8, 8))

        for bn in (layer.bn1, layer.bn2):
            np.testing.assert_allclose(
        ...
        assert layer.conv1.kernel.shape == (3, 3, 8, 8)
...
class TestNoProjectionBranchBuildsNothing:
    def test_shortcut_conv_and_bn_are_none_when_no_projection(self):
        layer = BasicBlock(filters=8, use_projection=False, name="no_proj")
        layer.build((B, 8, 8, 8))
...
class TestGradientFlow:
    def test_every_trainable_weight_receives_a_gradient(self, rng):
        layer = BasicBlock(filters=8, use_projection=True, stride=2, name="grad")
        x = tf.constant(rng.standard_normal((B, 8, 8, 8)).astype("float32"))
...
class TestDegenerateSpatialExtent:
    def test_one_by_one_input(self, rng):
        """0-length spatial extent is not constructible (every conv/pooling
        op in the block requires >= 1 spatial position); 1x1 with 'same'
        padding and stride 1 is the smallest valid degenerate case.
        """
        x = rng.standard_normal((B, 1, 1, 8)).astype("float32")
        layer = BasicBlock(filters=8, name="degenerate")
        out = layer(x)
        assert tuple(out.shape) == (B, 1, 1, 8)
        assert bool(keras.ops.all(keras.ops.isfinite(out)))
...
class TestDtypePolicy:
    def test_forward_pass_is_finite_under_every_policy(self, dtype_policy, rng):
        x = rng.standard_normal((B, 8, 8, 8)).astype("float32")
        layer = BasicBlock(filters=8, name=f"dtype_{dtype_policy}")
...
class TestXlaVersusEager:
    def test_jit_compiled_output_matches_eager(self, rng):
        x = rng.standard_normal((B, 8, 8, 8)).astype("float32")
        layer = BasicBlock(filters=8, name="jit_basic_block")
        ...
        # Two 3x3x8 convs (72 taps each) chained: bound conservatively on
        # float32 eps scaled by the accumulation length and output magnitude.
        n_accumulations = 2 * (3 * 3 * 8)
...
class TestKerasRoundTripExact:
    def test_round_trip_is_bit_identical(self, tmp_path, rng):
        x = rng.standard_normal((B, 8, 8, 8)).astype("float32")
        inp = keras.Input(shape=(8, 8, 8))
        out = BasicBlock(filters=8, use_projection=True, name="exact_rt")(inp)
        model = keras.Model(inp, out)

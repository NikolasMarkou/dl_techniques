...
class TestSaturatedMish:
    ...
    def test_invalid_alpha(self):
        ...
        with pytest.raises(ValueError):
            SaturatedMish(alpha=0.0)
        with pytest.raises(ValueError):
            SaturatedMish(alpha=-1.0)
    ...
    def test_serialization(self):
        """Test serialization and deserialization."""
        layer = SaturatedMish(alpha=2.0)
        config = layer.get_config()
        new_layer = SaturatedMish.from_config(config)
        assert isinstance(new_layer, SaturatedMish)
        assert new_layer.alpha == 2.0

# System Atlas
*Last refreshed: plan_2026-06-14_60541575 | 2026-06-14*
...
- **`layers/mixtures/`** -- differentiable soft-clustering sub-package. `RBFLayer`, `KMeansLayer` (centroid_initializer='orthonormal' lazy-resolved), `GMMLayer` (mean_initializer='orthonormal' lazy-resolved), `factory.py`. **105 PASS tests**. Serialization fixed (plan_2026-06-14_8c7365d0): `cluster_axis` now serializes the pre-build constructor value (`_cluster_axis_arg`) instead of the post-mutation positive form; `centroid_regularizer` in `KMeansLayer` now stored via `keras.regularizers.get()` for round-trip safety.
...
- **`layers/ffn/`** -- factory'd FFN sub-package: **15 registered types** (plan_2026-06-14_60541575). `gelu_tanh`/`GELUMLPFFN` is the 15th type. **Package `__init__` now exports all 15 classes + 5 factory symbols as strings** (`__all__` was class objects; 4 classes were unreachable -- fixed). **All 15 `build()` methods have `if self.built: return` as their first line** (was 3/15; sweep across 10 layers + SwinMLP/CountingFFN/Tversky real-bug fixes). **Real bugs fixed**: SwinMLP `fc2`-in-`build()` double-build weight loss; CountingFFN graph-unsafe Python branch on dynamic `ops.shape` (now static `_input_last_dim` captured in `build()`); `TverskyProjectionLayer.call()` missing `training=None` kwarg + fail-loud rank-2 guard in `build()`. **OrthoGLUFFN now has 16 dedicated tests** including `.keras` round-trip (latent tuple→list `ortho_reg_factor` round-trip bug fixed: `isinstance(..., (tuple, list))` + `tuple()` normalization). `FFN_REGISTRY['kan']` registers `KANLinear` with `optional_params` including `base_scaler_initializer` (default `'ones'`). Factory silently drops kwargs NOT listed in `optional_params` (factory.py). Deferred: callable-activation serialization in residual/mlp/swin (rare path); Tversky rank>2; KAN `update_grid_from_samples` eager-only (by design).
